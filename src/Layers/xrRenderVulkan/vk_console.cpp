...
// 3D Fluid system (Phase 0)
u32 ps_r_fluid_quality = 1;
xr_token qfluid_quality_token[] = {
    {"low", 0}, {"medium", 1}, {"high", 2}, {0, 0}
};

u32 ps_r_fluid_grid_size = 128;
xr_token qfluid_grid_token[] = {
    {"64", 64}, {"128", 128}, {"256", 256}, {0, 0}
};

// ============================================================================
// External variables (defined elsewhere in the engine)
// ============================================================================
extern int psSkeletonUpdate;
extern float r__dtex_range;
float OLES_SUN_LIMIT_27_01_07 = 100.f; // sun_far (defined here for Vulkan, R4 has its own)
float r_dtex_range = 50.f;             // detail texture range (defined here for Vulkan)

// ============================================================================
// Common flags and settings
// ============================================================================
Flags32 ps_r__common_flags = {0};
extern int ps_r__Supersample; // defined in xrEngine/xr_ioc_cmd.cpp
...
// ============================================================================
// Console command classes (Vulkan-compatible versions)
// These are defined locally in the DX renderer's xrRender_console.cpp and
// not exposed via any header, so we replicate them here for Vulkan.
// ============================================================================
...
public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		// TODO: Vulkan anisotropy control
	}
...
public:
	...
	virtual void Info(TInfo& I) { xr_sprintf(I, "vector3 in range [%f,%f,%f]-[%f,%f,%f]", min.x, min.y, min.z, max.x, max.y, max.z); }
};
...
void xrRender_initconsole()
{
    ...
    // NOTE: This registration is copied from xrRender_console.cpp to ensure
    // all console variables are accessible from Lua scripts via get_console()
    ...
    CMD4(CCC_Float, "r__detail_density", &ps_current_detail_density, 0.04f, 1.f);
    ...
    CMD4(CCC_Integer, "r__nightvision", &ps_r2_nightvision, 0, 3);
    CMD4(CCC_Integer, "r__fakescope", &scope_fake_enabled, 0, 1);
    CMD4(CCC_Integer, "r__3Dfakescope", &scope_3D_fake_enabled, 0, 1);
    CMD4(CCC_Integer, "r__heatvision", &ps_r2_heatvision, 0, 1);
    CMD3(CCC_Mask, "r2_terrain_z_prepass", &ps_r2_ls_flags, R2FLAG_TERRAIN_PREPASS);
    ...
    CMD3(CCC_Token, "r_fluid_quality", &ps_r_fluid_quality, qfluid_quality_token);
    CMD3(CCC_Token, "r_fluid_grid_size", &ps_r_fluid_grid_size, qfluid_grid_token);
    ...
    CMD3(CCC_Mask, "r__clear_models_on_unload", &psDeviceFlags2, rsClearModels);
    CMD3(CCC_Mask, "r__use_precompiled_shaders", &psDeviceFlags2, rsPrecompiledShaders);
