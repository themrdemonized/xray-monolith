...
// vk_occRasterizer.cpp: Vulkan port of occRasterizer (software HOM rasterizer)
// API-independent software rasterizer for hierarchical occlusion mapping
//////////////////////////////////////////////////////////////////////
...
#ifdef DEBUG
// Debug flag for detailed HOM visualization
extern BOOL bDebug;
#endif

// Note: No global Raster instance in Vulkan version.
// Each vkCHOM has its own occRasterizer instance.
...
{
#ifdef DEBUG
	// Visualize HOM depth buffer as 3D boxes in world space
	// Each pixel in the 64x64 depth buffer becomes a colored box
	// Box color represents depth (near=blue, far=red)

	// Check if HOM depth visualization is enabled
	// TODO: Add console variable ps_r2_ls_flags_ext with R_FLAGEXT_HOM_DEPTH_DRAW flag
	// For now, always disabled until we have proper flag support
	const bool bHOM_DEPTH_DRAW = false;
	...
	// Build 3D box data for each depth buffer pixel
	for (int i = 0; i < occ_dim_0; ++i)
	{
		for (int j = 0; j < occ_dim_0; ++j)
		{
			if (bDebug)
			{
				...
				// Convert from depth buffer space to NDC space
				quad.set((float)j - occ_dim_0 / 2.f,
				         -((float)i - occ_dim_0 / 2.f),
				         (float)bufDepth_0[i][j] / occQ_s32);

				// Project back to view space using projection matrix
				float z = -Device.mProject._43 / (float)(Device.mProject._33 - quad.z);
				left_top.set(quad.x * z / Device.mProject._11 / (occ_dim_0 / 2.f),
				             quad.y * z / Device.mProject._22 / (occ_dim_0 / 2.f),
				             z);
				right_bottom.set((quad.x + 1) * z / Device.mProject._11 / (occ_dim_0 / 2.f),
				                 (quad.y + 1) * z / Device.mProject._22 / (occ_dim_0 / 2.f),
				                 z);

				box_center.set((right_bottom.x + left_top.x) / 2,
			...
			// Render the box for this pixel
			pixel_box& tmp = dbg_pixel_boxes[i * occ_dim_0 + j];
			Fmatrix Transform;
			Transform.identity();
			Transform.translate(tmp.center);

			// TODO: Device.SetNearer(TRUE) - set depth bias for drawing

			// TODO: Set selection shader
			// TODO: Calculate color based on depth (color gradient from blue to red)
			u32 color = color_rgba(255, 255, 255, 128); // White semi-transparent for now

			// Draw oriented bounding box for this depth pixel
			RCache.dbg_DrawOBB(Transform, tmp.radius, color);

			// TODO: Device.SetNearer(FALSE) - restore normal depth
		}
	}
#endif
}
...
IC BOOL test_Level(occD* depth, int dim, float _x0, float _y0, float _x1, float _y1, occD z)
{
	...
	// Note: MT_SYNC is called by vkCHOM before test() is called
