...
#include "../xrRender/AnimationKeyCalculate.h"
...
void CKinematicsAnimated::Load(const char* N, IReader* data, u32 dwFlags)
{
	...
	const auto loadOMF = [&](LPCSTR _path)
	{
		...
		if (!FS.exist(fn, "$level$", _path))
		{
			...
			{
				Msg("![Vulkan] loadOMF: Can't find motion file '%s'", _path);
		...
		if (!g_pMotionsContainer->has(_path)) //optimize fs operations
		{
			...
			if (!MS)
			{
				Msg("![Vulkan] loadOMF: Failed to open file '%s'", fn);
				m_Motions.pop_back();
				return;
			}
...
{
	return xr_new<CKinematicsAnimated>();
}
