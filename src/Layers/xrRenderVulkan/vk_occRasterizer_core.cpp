...
static occRasterizer* g_pRaster = nullptr;  // Current rasterizer instance for helper functions
...
void i_scan(int curY, float leftX, float lhx, float rightX, float rhx, float startZ, float endZ)
{
	...
	occTri** pFrame = g_pRaster->get_frame();
	float* pDepth = g_pRaster->get_depth();
...
u32 occRasterizer::rasterize(occTri* T)
{
	...
	currentTri = T;
	dwPixels = 0;
	g_pRaster = this;  // Set current rasterizer for helper functions
