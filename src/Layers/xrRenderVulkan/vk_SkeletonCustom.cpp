...
void CKinematics::DebugRender(Fmatrix& XFORM)
{
	...
	for (u32 i=0; i<dbgLines.size(); i+=2)	{
		...
		// TODO Vulkan: RCache.dbg_DrawLINE(XFORM,P1,P2,D3DCOLOR_XRGB(0,255,0));

		Fmatrix M;
		M.mul_43(XFORM,M2);
		// TODO Vulkan: RCache.dbg_DrawOBB(M,H1,D3DCOLOR_XRGB(255,255,255));
		// TODO Vulkan: RCache.dbg_DrawOBB(M,H2,D3DCOLOR_XRGB(255,255,255));
	}
	...
	for (u32 b=0; b<bones->size(); b++)
	{
		...
		// TODO Vulkan: RCache.dbg_DrawOBB(W,obb.m_halfsize,D3DCOLOR_XRGB(0,0,255));
	}
}
#endif
...
void CKinematics::Visibility_Update()
{
	...
	for (u32 c_it = 0; c_it < children.size(); c_it++)
	{
		...
		if (!_c->has_visible_bones())
		{
			...
			children_invisible.push_back((vkRender_Visual*)children[c_it]);
