...
void CBlender_Model::Compile(CBlender_Compile& C)
{
	...
	else
	{
		...
		case SE_R1_NORMAL_HQ:
			...
			if (oBlend.value) C.r_Pass(vsname, psname,TRUE,TRUE,TRUE,TRUE, D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA,
			                           TRUE, oAREF.value);
		...
		case SE_R1_LPOINT:
			...
			psname = "add_point";
			if (oBlend.value) C.r_Pass(vsname, psname,FALSE,TRUE,FALSE,TRUE, D3DBLEND_ONE, D3DBLEND_ONE,TRUE,
			                           oAREF.value);
