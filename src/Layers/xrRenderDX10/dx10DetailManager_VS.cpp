		...
		for (u32 O = 0; O < objects.size(); O++)
		{
			...
			for (; _vI != _vE; _vI++)
			{
				...
				for (; _iI != _iE; _iI++)
				{
					...
					// 3x4 transform rows (scale pre-multiplied in UpdateVisibleM) + packed terrain-normal/alpha + sun/hemi
...
{
	// still details (var 0) don't cast shadows
	if (RImplementation.phase == CRender::PHASE_SMAP && var_id == 0)
		return;

	// no point drawing grass if the outdoor sector isn't visible this frame
