		...
		// Handle the first face in the strip
		{
			...
			else
			{
				...
				if (NextIsCW(stripIndices.size() - accountForNegatives) != IsCW(
					strip->m_faces[0], tFirstFace.m_v0, tFirstFace.m_v1))
				{
					stripIndices.push_back(tFirstFace.m_v0);
				}
			}
                                 ...
                                 int numSamples)
{
	...
	while (!done)
	{
		...
		for (i = 0; i < numSamples; i++)
		{
			...
			NvStripInfo* strip01 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge01, true), stripId++,
			                                           experimentId++);
			...
			NvStripInfo* strip10 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge10, false), stripId++,
			                                           experimentId++);
			...
			NvStripInfo* strip12 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge12, true), stripId++,
			                                           experimentId++);
			...
			NvStripInfo* strip21 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge21, false), stripId++,
			                                           experimentId++);
			...
			NvStripInfo* strip20 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge20, true), stripId++,
			                                           experimentId++);
			...
			NvStripInfo* strip02 = xr_new<NvStripInfo>(NvStripStartInfo(nextFace, edge02, false), stripId++,
			                                           experimentId++);
