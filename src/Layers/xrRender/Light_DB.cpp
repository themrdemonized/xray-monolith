...
void CLight_DB::Load(IReader* fs)
{
	...
	sun_original = NULL;
	sun_adapted = NULL;
	rain_light = Create();   // pip: persistent rain shadow light
	sun_cascades.clear();    // pip: rebuilt below (one light per cascade)
	{
		...
		for (u32 i = 0; i < count; i++)
		{
			...
			if (Ldata.type == D3DLIGHT_DIRECTIONAL)
			{
				...
				// copy to env-sun, pip: one light per sun cascade
				...
				// all cascades are identical at load and Update reassigns every one, use a deterministic in-range element
				sun_adapted = sun_cascades.back();
			}
			else
			{
				Fvector tmp_D, tmp_R;
...
//			Log("sect_name", E.sect_name.c_str());
		...
		// pip: update every sun cascade light identically
		for (auto s : sun_cascades)
		{
			...
			s->set_position(AP);
			s->set_color(E.sun_color.x * ps_r2_sun_lumscale * ps_r2_sun_lumscale_color.x,
