	...
	case M_EVENT:
		{
			...
			case GE_DESTROY:
				{
					xr_sprintf(tmp_string, "--- CL_EVENT [%7u][%5u]: GE_DESTROY", msg_type.msg_receive_time,
					           msg_type.dest_obj_id);
				}
				break;
			case GE_OWNERSHIP_TAKE:
				{
					...
					xr_sprintf(tmp_string, "--- CL_EVENT [%7u][%5u]: GE_OWNERSHIP_TAKE    [%d]",
					           msg_type.msg_receive_time, msg_type.dest_obj_id, tmp_id_what);
				}
				break;
			case GE_OWNERSHIP_REJECT:
				{
					u16 tmp_id_what;
					packet.r_u16(tmp_id_what);
					xr_sprintf(tmp_string, "--- CL_EVENT [%7u][%5u]: GE_OWNERSHIP_REJECT  [%d]",
					           msg_type.msg_receive_time, msg_type.dest_obj_id, tmp_id_what);
				}
				break;
			default:
	...
	case M_GAMEMESSAGE:
		{
			...
			default:
				{
					xr_sprintf(tmp_string, "--- GM_EVENT [%7u]: GAME_EVENT_ID=[%d]", msg_type.msg_receive_time,
