...
IC void CCar::SWheel::applywheelCollisionParams(const dxGeomUserData* ud, bool& do_colide, dContact& c,
...
void CCar::SWheel::WheellCollisionCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                                           SGameMtl* material_2)
{
...
void CCar::SWheel::Load(LPCSTR section)
{
	...
	{
		collision_params.damping_factor = READ_IF_EXISTS(ini, r_float, section, "damping_factor",
		                                                 collision_params.damping_factor);
		collision_params.spring_factor = READ_IF_EXISTS(ini, r_float, section, "spring_factor",
		                                                collision_params.spring_factor);
		collision_params.mu_factor = READ_IF_EXISTS(ini, r_float, section, "friction_factor",
...
void CCar::SWheelSteer::Init()
{
	...
	{
	case jtWheel:
		pos_right = bone_map.find(pwheel->bone_id)->second.element->mXFORM.i.y;
		//.dotproduct(pwheel->car->m_root_transform.j);
		break;
