			...
			case KT_HIT:
				{
					...
					case SKT_HEADSHOT:
						{
							Player_AddExperience(
								pKiller, READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "headshot", 0));
							Player_AddBonusMoney(
								pKiller, READ_IF_EXISTS(pSettings, r_s32, "mp_bonus_money", "headshot", 0),
								SKT_HEADSHOT);
						}
						break;
					case SKT_EYESHOT:
						{
							Player_AddExperience(
								pKiller, READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "eyeshot", 0));
							Player_AddBonusMoney(
					...
					case SKT_BACKSTAB:
						{
							Player_AddExperience(
								pKiller, READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "backstab", 0));
							Player_AddBonusMoney(
								pKiller, READ_IF_EXISTS(pSettings, r_s32, "mp_bonus_money", "backstab", 0),
								SKT_BACKSTAB);
						}
						break;
					default:
								...
								case CLSID_OBJECT_W_KNIFE:
									{
										Player_AddExperience(
											pKiller,
											READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "knife_kill", 0));
										Player_AddBonusMoney(
											pKiller,
											READ_IF_EXISTS(pSettings, r_s32, "mp_bonus_money", "knife_kill", 0),
											SKT_KNIFEKILL);
									}
									break;
								};
...
void game_sv_CaptureTheArtefact::DropArtefact(CSE_ActorMP* aOwner, CSE_ALifeItemArtefact* artefact,
                                              Fvector const* dropPosition)
{
...
void game_sv_CaptureTheArtefact::ReSpawnArtefacts()
{
	...
	for (TeamsMap::iterator ti = teams.begin(); ti != te; ti++)
	{
		...
		CSE_ALifeItemArtefact* tempSvEntity = smart_cast<CSE_ALifeItemArtefact*>(
			spawn_begin(ti->second.artefactName.c_str()));
		...
		ti->second.artefact = smart_cast<CSE_ALifeItemArtefact*>(
			spawn_end(tempSvEntity, m_server->GetServerClient()->ID));
...
BOOL game_sv_CaptureTheArtefact::OnTouchItem(CSE_ActorMP* actor, CSE_Abstract* item)
{
	...
	if ((item->m_tClassID == CLSID_OBJECT_PLAYERS_BAG) && (item->ID_Parent == 0xffff))
	{
			...
			while (!item->children.empty())
			{
				...
				if (e_child_item)
				{
					if (!OnTouch(actor->ID, e_child_item->ID, FALSE))
					{
						...
						m_server->Process_event_reject(P, m_server->GetServerClient()->ID, 0, item->ID,
						                               e_child_item->ID);
						continue;
					}
				}
		...
		{
			Player_AddBonusMoney(actor->owner->ps, READ_IF_EXISTS(pSettings, r_s32, "mp_bonus_money", "pda_taken", 0),
			                     SKT_PDA);
		};
