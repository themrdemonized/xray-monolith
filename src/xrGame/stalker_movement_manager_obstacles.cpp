...
void stalker_movement_manager_obstacles::move_along_path_impl(CPHMovementControl* movement_control,
                                                              Fvector& dest_position, float time_delta)
{
#ifndef MASTER_GOLD
...
void stalker_movement_manager_obstacles::move_along_path(CPHMovementControl* movement_control, Fvector& dest_position,
                                                         float time_delta)
{
...
)
{
	...
	case LevelGraph::eLineIntersectionCollinear:
		{
			Fvector2 const as_af = Fvector2().sub(Fvector2().set(a_second.x, a_second.z),
			                                      Fvector2().set(a_first.x, a_first.z));
			Fvector2 const bf_af = Fvector2().sub(Fvector2().set(b_first.x, b_first.z),
			                                      Fvector2().set(a_first.x, a_first.z));
...
float stalker_movement_manager_obstacles::is_going_through(Fmatrix const& matrix, Fvector const& vector,
