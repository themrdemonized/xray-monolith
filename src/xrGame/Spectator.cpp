...
void CSpectator::IR_OnKeyboardPress(int cmd)
{
	...
	case kACCEL:
		...
		break;
	case kCAM_1: if (cam_active == eacFreeFly && SelectNextPlayerToLook(false)) cam_Set(eacFirstEye);
		break;
	case kCAM_2: if (cam_active == eacFreeFly && SelectNextPlayerToLook(false)) cam_Set(eacLookAt);
		break;
	case kCAM_3: if (cam_active == eacFreeFly && SelectNextPlayerToLook(false)) cam_Set(eacFreeLook);
		break;
