...
void CCar::SCarSound::Init()
{
	...
	if (ini->section_exist("car_sound") && ini->line_exist("car_sound", "snd_volume"))
	{
		...
		snd_engine_start.create(
			READ_IF_EXISTS(ini, r_string, "car_sound", "engine_start", "car\\test_car_start"), st_Effect,
			sg_SourceType);
		...
		engine_start_delay = iFloor(
			(snd_engine_start._handle() ? iFloor(snd_engine_start.get_length_sec() * 1000.0f) : 1.f) *
			fengine_start_delay);
...
void CCar::SCarSound::Update()
{
	...
	switch (eCarSound)
	{
	case sndStarting: UpdateStarting();
		break;
	case sndDrive: UpdateDrive();
		break;
	case sndStalling: UpdateStalling();
		break;
