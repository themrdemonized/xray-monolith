...
IC CLASS_ID CBaseFunction::clsid_member() const
{
	...
	else
	{
		...
		const CSE_ALifeDynamicObject* l_tpALifeDynamicObject = smart_cast<const CSE_ALifeDynamicObject*>(
			ef_storage().alife().member());
...
float CPersonalHealthFunction::ffGetValue()
{
	...
	{
		const CSE_ALifeMonsterAbstract* l_tpALifeMonsterAbstract = smart_cast<const CSE_ALifeMonsterAbstract*>(
			ef_storage().alife().member());
		...
		if (tpInventoryOwner)
		{
			...
			for (; I <= E; ++I)
			{
				...
				if (iitem)
				{
					...
					if (tpCustomWeapon && (tpCustomWeapon->GetSuitableAmmoTotal(true) > tpCustomWeapon->GetAmmoMagSize()
						/ 10))
					{
...
float CPersonalMaxHealth::ffGetValue()
{
	CSE_ALifeMonsterAbstract* l_tpALifeMonsterAbstract = smart_cast<CSE_ALifeMonsterAbstract*>(
		ef_storage().alife().member());
	VERIFY3(l_tpALifeMonsterAbstract, "Invalid object passed to the evaluation function ", m_caName);
	const CSE_ALifeGroupAbstract* l_tpALifeGroupAbstract = smart_cast<const CSE_ALifeGroupAbstract*>(
		ef_storage().alife().member());
...
#pragma todo("Dima to Dima : Append ItemDeterioration with non-ALife non-weapon branch")
	...
	{
		const CSE_ALifeInventoryItem* l_tpALifeInventoryItem = smart_cast<const CSE_ALifeInventoryItem*>(
			ef_storage().alife().member_item());
...
#pragma todo("Dima to Dima : Append EquipmentPreference with non-ALife branch")
	...
	{
		CSE_ALifeHumanAbstract* l_tpALifeHumanAbstract = smart_cast<CSE_ALifeHumanAbstract*>(
			ef_storage().alife().member());
		R_ASSERT2(l_tpALifeHumanAbstract, "Non-human object in EquipmentPreference evaluation function");
		return (l_tpALifeHumanAbstract->brain().m_cpEquipmentPreferences[ef_storage()
		                                                                 .m_pfEquipmentType->dwfGetDiscreteValue()]);
	}
}
...
#pragma todo("Dima to Dima : Append MainWeaponPreference with non-ALife branch")
	...
	else
	{
		CSE_ALifeHumanAbstract* l_tpALifeHumanAbstract = smart_cast<CSE_ALifeHumanAbstract*>(
			ef_storage().alife().member());
		R_ASSERT2(l_tpALifeHumanAbstract, "Non-human object in EquipmentPreference evaluation function");
		return (l_tpALifeHumanAbstract->brain().m_cpMainWeaponPreferences[ef_storage()
		                                                                  .m_pfMainWeaponType->dwfGetDiscreteValue(
			                                                                  iFloor(
				                                                                  ef_storage()
				                                                                  .m_pfMainWeaponType->
				                                                                  ffGetMaxResultValue() + .5f))]);
	}
}
#else
float CEquipmentPreference::ffGetValue()
...
u32 CWeaponAmmoCount::dwfGetDiscreteValue(u32 dwDiscretizationValue)
{
	...
	{
		const CSE_ALifeItemWeapon* l_tpALifeItemWeapon = smart_cast<const CSE_ALifeItemWeapon*>(
			ef_storage().alife().member_item());
