...
namespace award_system
{
	...
	void game_state_accumulator::OnBullet_Hit(CObject const* hitter, CObject const* victim, CObject const* weapon,
	                                          u16 const bone)
	{
	...
	void game_state_accumulator::OnPlayerKilled(u16 killer_id, u16 target_id, u16 weapon_id,
	...
	u16 game_state_accumulator::get_active_weapon_of_player(game_PlayerState* player)
	{
		...
		CInventoryItem const* tmp_inv_item = tmp_active_slot != NO_ACTIVE_SLOT
			                                     ? tmp_actor->inventory().ItemFromSlot(tmp_active_slot)
			                                     : NULL;
