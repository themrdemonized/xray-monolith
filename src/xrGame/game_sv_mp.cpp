...
void game_sv_mp::ChargeGrenades(CSE_ALifeItemWeapon* weapon, LPCSTR grenade_string,
                                game_PlayerState::PLAYER_ITEMS_LIST& playerItems)
{
...
void game_sv_mp::UpdateVote()
{
	...
	else
	{
		if (g_sv_mp_bCountParticipants)
			VoteSucceed = (float(vote_stats.NumAgreed) / float(vote_stats.NumParticipated + NumAgainst)) >=
				g_sv_mp_fVoteQuota;
		else
...
void game_sv_mp::RejectGameItem(CSE_Abstract* entity)
{
	...
	VERIFY2(e_parent,
	        make_string( "RejectGameItem: parent not found. entity_id = [%d], parent_id = [%d]", entity->ID, entity->
		        ID_Parent ).c_str());
	if (!e_parent)
	{
...
void game_sv_mp::DestroyAllPlayerItems(ClientID id_who) //except rukzak
{
	     ...
	     ii != iie; ++ii)
	{
		//		VERIFY(*ii);
		R_ASSERT2(
			*ii, make_string("PIItem in player`s inventory not found. Destroy all items of actor[%d]", ps->GameID).c_str
			());
