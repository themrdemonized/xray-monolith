...
void game_cl_mp::OnChatMessage(NET_Packet* P)
{
	...
	switch (team)
	{
	case 0: Msg("%s: %s : %s", *st.translate("mp_chat"), PlayerName.c_str(), ChatMsg.c_str());
		break;
	case 1: Msg("- %s: %s : %s", *st.translate("mp_chat"), PlayerName.c_str(), ChatMsg.c_str());
		break;
