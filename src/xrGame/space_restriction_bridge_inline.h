...
#pragma todo("Dima to Dima : _Warning : this place can be optimized in case of a slowdown")
...

	{
		...
		for (u32 i = 0; i < 5; ++i)
		{
			...
			switch (i)
			{
			case 0: current.P.set(center.x + offset, center.y, center.z + offset);
				break;
			case 1: current.P.set(center.x + offset, center.y, center.z - offset);
				break;
			case 2: current.P.set(center.x - offset, center.y, center.z + offset);
				break;
			case 3: current.P.set(center.x - offset, center.y, center.z - offset);
				break;
			case 4: current.P.set(center.x, center.y, center.z);
				break;
			default: NODEFAULT;
			}
			if (i < 4)
