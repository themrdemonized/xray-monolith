...
CUIDemoPlayControl::CUIDemoPlayControl()
{
	...
	m_decrease_speed_btn = xr_new<CUI3tButton>();
	AttachChild(m_decrease_speed_btn);
	m_play_pause_btn = xr_new<CUI3tButton>();
	AttachChild(m_play_pause_btn);
	m_increase_speed_btn = xr_new<CUI3tButton>();
	AttachChild(m_increase_speed_btn);
	m_rewind_until_btn = xr_new<CUI3tButton>();
	AttachChild(m_rewind_until_btn);
	m_repeat_rewind_btn = xr_new<CUI3tButton>();
	AttachChild(m_repeat_rewind_btn);
	m_static_demo_status = xr_new<CUITextWnd>();
...
void CUIDemoPlayControl::Init()
{
	...
	AddCallback(m_play_pause_btn, BUTTON_CLICKED,
	            CUIWndCallback::void_function(this, &CUIDemoPlayControl::OnPlayPause));
	...
	AddCallback(m_rewind_until_btn, BUTTON_CLICKED,
	            CUIWndCallback::void_function(this, &CUIDemoPlayControl::OnRewindUntil));
	...
	AddCallback(m_repeat_rewind_btn, BUTTON_CLICKED,
	            CUIWndCallback::void_function(this, &CUIDemoPlayControl::OnRepeatRewind));
...
void CUIDemoPlayControl::OnRepeatRewind(CUIWindow* w, void* d)
{
	...
	case eRewindUntilArtTake:
		{
			rewind_result = m_demo_play_control->rewind_until(demoplay_control::on_artefactcapturing,
			                                                  m_last_rewind_target, tmpcb);
		};
		break;
	case eRewindUntilArtDrop:
		{
			rewind_result = m_demo_play_control->rewind_until(demoplay_control::on_artefactloosing,
			                                                  m_last_rewind_target, tmpcb);
		};
		break;
	case eRewindUntilArtDeliver:
		{
			rewind_result = m_demo_play_control->rewind_until(demoplay_control::on_artefactdelivering,
			                                                  m_last_rewind_target, tmpcb);
		};
		break;
	default:
