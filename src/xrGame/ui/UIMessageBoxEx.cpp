...
void CUIMessageBoxEx::InitMessageBox(LPCSTR xml_template)
{
	...
	AddCallback(m_pMessageBox, MESSAGE_BOX_YES_CLICKED,
	            CUIWndCallback::void_function(this, &CUIMessageBoxEx::OnOKClicked));
	CUIMessageBox::E_MESSAGEBOX_STYLE style = m_pMessageBox->GetBoxStyle();
	if (style == CUIMessageBox::MESSAGEBOX_YES_NO || style == CUIMessageBox::MESSAGEBOX_QUIT_WINDOWS || style ==
		CUIMessageBox::MESSAGEBOX_QUIT_GAME)
		AddCallback(m_pMessageBox, MESSAGE_BOX_NO_CLICKED,
		            CUIWndCallback::void_function(this, &CUIMessageBoxEx::OnNOClicked));
}
