...
struct eq_name_state_addon_comparer
{
	...
	eq_name_state_addon_comparer(const shared_str& _name, SBuyItemInfo::EItmState _state, u8 ad): m_name(_name),
	                                                                                              m_state(_state),
	                                                                                              m_addons(ad)
	{
	}
...
{
	return (u32)std::count_if(m_all_items.begin(), m_all_items.end(),
	                          eq_name_state_addon_comparer(name_sect, state, addon));
}
...
void CUIMpTradeWnd::CleanUserItems()
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		else if (i == 1) _state = SBuyItemInfo::e_sold;
		else
...
void CUICellItemTradeMenuDraw::OnDraw(CUICellItem* cell)
{
	...
	bool b_can_buy_rank = m_trade_wnd->CheckBuyPossibility(m_info_item->m_name_sect, CUIMpTradeWnd::bf_check_rank_restr,
	                                                       true);
	...
	bool b_can_buy_money = m_trade_wnd->CheckBuyPossibility(m_info_item->m_name_sect, CUIMpTradeWnd::bf_check_money,
	                                                        true);
	if (!b_can_buy_money)
	{
