...
void CUISkinSelectorWnd::SetVisibleForBtn(ESKINMENU_BTN btn, bool state)
{
	switch (btn)
	{
	case SKIN_MENU_BACK: this->m_pBtnBack->SetVisible(state);
		break;
	case SKIN_MENU_SPECTATOR: this->m_pBtnSpectator->SetVisible(state);
		break;
	case SKIN_MENU_AUTOSELECT: this->m_pBtnAutoSelect->SetVisible(state);
		break;
	default:
