...
u32 CMonsterHome::get_place_in_max_home_to_direction(Fvector to_direction)
{
	...
	if (input_node != u32(-1))
		m_object->control().path_builder().get_node_in_radius(input_node, 1, (m_radius_max - m_radius_middle) / 2, 5,
		                                                      result);
	...
	if (result == u32(-1))
	{
		...
		else
		{
			...
			if (input_node != u32(-1))
				m_object->control().path_builder().get_node_in_radius(input_node, 1,
				                                                      (m_radius_max - m_radius_middle) / 2, 5, result);

			if (result == u32(-1))
			{
				if (ai().level_graph().valid_vertex_id(input_node) && m_object
...
u32 CMonsterHome::get_place()
{
	...
	{
		if (ai().level_graph().valid_vertex_id(m_level_vertex_id))
			m_object->control().path_builder().get_node_in_radius(m_level_vertex_id, m_radius_min,
			                                                      m_radius_min + (m_radius_max - m_radius_min) / 2, 5,
			                                                      result);

		if (result == u32(-1))
		{
			m_object->control().path_builder().get_node_in_radius(m_object->ai_location().level_vertex_id(), 5, 15, 5,
			                                                      result);
			if (result == u32(-1))
			{
				m_object->control().path_builder().get_node_in_radius(m_object->ai_location().level_vertex_id(), 2, 3,
				                                                      10, result);
				if (result == u32(-1))
...
u32 CMonsterHome::get_place_in_cover()
{
	if (!m_path)
	{
		if (ai().level_graph().valid_vertex_id(m_level_vertex_id))
		{
			const CCoverPoint* point = m_object->CoverMan->find_cover(
				ai().level_graph().vertex_position(m_level_vertex_id),
				ai().level_graph().vertex_position(m_level_vertex_id),
