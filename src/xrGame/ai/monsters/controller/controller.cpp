...
void CController::Load(LPCSTR section)
{
	...
	::Sound->create(control_start_sound, pSettings->r_string(section, "sound_control_start"), st_Effect,
	                SOUND_TYPE_WORLD);
...
void CController::UpdateControlled()
{
	...
	if (EnemyMan.get_enemy())
	{
		CControlledEntityBase* entity = smart_cast<CControlledEntityBase *>(
			const_cast<CEntityAlive *>(EnemyMan.get_enemy()));
		if (entity)
		{
...
void CController::set_controlled_task(u32 task)
{
	...
	const CEntity* object = ((((ETask)task) == eTaskNone)
		                         ? 0
		                         : ((((ETask)task) == eTaskFollow) ? this : EnemyMan.get_enemy()));
...
void CController::reload(LPCSTR section)
{
	...
	com_man().ta_fill_data(anim_triple_control, "stand_sit_down_attack_0", "control_attack_0", "sit_stand_up_attack_0",
	                       true, false);
}
...
void CController::reinit()
{
	...
	control().path_builder().detail().add_velocity(MonsterMovement::eControllerVelocityParameterMoveFwd,
	                                               CDetailPathManager::STravelParams(
		                                               m_velocity_move_fwd.velocity.linear,
	...
	control().path_builder().detail().add_velocity(MonsterMovement::eControllerVelocityParameterMoveBkwd,
	                                               CDetailPathManager::STravelParams(
		                                               m_velocity_move_bkwd.velocity.linear,
...
void CController::control_hit()
{
	...
	Actor()->Cameras().AddCamEffector(xr_new<CMonsterEffectorHit>(m_control_effector.ce_time,
	                                                              m_control_effector.ce_amplitude,
	                                                              m_control_effector.ce_period_number,
	...
	Actor()->Cameras().AddPPEffector(xr_new<CMonsterEffector>(m_control_effector.ppi, m_control_effector.time,
	                                                          m_control_effector.time_attack,
...
{
	for (u32 i = 0; i < m_controlled_objects.size(); i++) smart_cast<CControlledEntityBase *>(m_controlled_objects[i])->
		free_from_control();
...
void CController::TranslateActionToPathParams()
{
	...
	u32 des_mask = (m_bDamaged
		                ? MonsterMovement::eVelocityParameterWalkDamaged
		                : MonsterMovement::eVelocityParameterWalkNormal);
...
void CController::HitEntity(const CEntity* pEntity, float fDamage, float impulse, Fvector& dir,
                            ALife::EHitType hit_type, bool draw_hit_marks)
{
