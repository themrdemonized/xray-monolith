...
void CAI_Bloodsucker::Load(LPCSTR section)
{
	...
	else
	{
		...
		anim().AddAnim(eAnimStandTurnRight, "stand_turn_rs_", -1, &velocity_turn, PS_STAND, "fx_run_f", "fx_stand_b",
		               "fx_stand_l", "fx_stand_r");
		anim().AddAnim(eAnimSleep, "lie_sleep_", -1, &velocity_none, PS_LIE, "fx_run_f", "fx_stand_b", "fx_stand_l",
		               "fx_stand_r");
		...
		anim().AddAnim(eAnimRun, "stand_run_", -1, &velocity_run, PS_STAND, "fx_run_f", "fx_stand_b", "fx_stand_l",
		               "fx_stand_r");
		...
		anim().AddAnim(eAnimCheckCorpse, "stand_check_corpse_", -1, &velocity_none, PS_STAND, "fx_run_f", "fx_stand_b",
		               "fx_stand_l", "fx_stand_r");
		anim().AddAnim(eAnimEat, "sit_eat_", -1, &velocity_none, PS_SIT, "fx_run_f", "fx_stand_b", "fx_stand_l",
		               "fx_stand_r");
...
void CAI_Bloodsucker::reload(LPCSTR section)
{
	...
	sound().add(pSettings->r_string(section, "Sound_Vampire_Sucking"), DEFAULT_SAMPLE_COUNT,
	            SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 3, MonsterSound::eBaseChannel,
	            eVampireSucking, "bip01_head");
	...
	sound().add(pSettings->r_string(section, "Sound_Vampire_StartHunt"), DEFAULT_SAMPLE_COUNT,
	            SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 5, MonsterSound::eBaseChannel,
	            eVampireStartHunt, "bip01_head");
...
void CAI_Bloodsucker::animation_end_jump(CBlend* B)
{
	...
	((CAI_Bloodsucker*)B->CallbackParam)->jump(((CAI_Bloodsucker*)B->CallbackParam)->j_position,
	                                           ((CAI_Bloodsucker*)B->CallbackParam)->j_factor);
}
...
{
	return (int)m_hits_before_vampire >= (int)m_sufficient_hits_before_vampire +
		m_sufficient_hits_before_vampire_random;
}
