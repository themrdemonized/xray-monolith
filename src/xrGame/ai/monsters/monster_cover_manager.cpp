...
void CCoverEvaluator::setup(CBaseMonster* object, const Fvector& position, float min_pos_distance,
...
const CCoverPoint* CMonsterCoverManager::find_cover(const Fvector& position, float min_pos_distance,
                                                    float max_pos_distance, float deviation)
{
...
const CCoverPoint* CMonsterCoverManager::find_cover(const Fvector& src_pos, const Fvector& dest_pos,
...
{
	float angle = ai().level_graph().vertex_high_cover_angle(m_object->ai_location().level_vertex_id(),deg(10),
	                                                         std::greater<float>());
