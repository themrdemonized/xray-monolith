...
void CControlAnimationBase::on_event(ControlCom::EEventType type, ControlCom::IEventData* data)
{
	switch (type)
	{
	case ControlCom::eventAnimationEnd: select_animation(true);
		m_state_attack = false;
		break;
	case ControlCom::eventAnimationSignal:
		{
			SAnimationSignalEventData* event_data = (SAnimationSignalEventData *)data;
			if (event_data->event_id == CControlAnimation::eAnimationHit) check_hit(
				event_data->motion, event_data->time_perc);
			break;
		}
	}
}
...
void CControlAnimationBase::FX_Play(EHitSide side, float amount)
{
	...
	case eSideFront: p_str = &anim_it->fxs.front;
		break;
	case eSideBack: p_str = &anim_it->fxs.back;
		break;
	case eSideLeft: p_str = &anim_it->fxs.left;
		break;
	case eSideRight: p_str = &anim_it->fxs.right;
		break;
	}
...
EAction CControlAnimationBase::GetActionFromPath()
{
	...
	u32 cur_point_velocity_index = m_object->movement().detail().path()[m_object
	                                                                    ->movement().detail().curr_travel_point_index()]
		.velocity;
...
CMotionDef* CControlAnimationBase::get_motion_def(SAnimItem* it, u32 index)
{
	...
	const MotionID& motion_id = skeleton_animated->ID_Cycle_Safe(
		strconcat(sizeof(s2), s2, *it->target_name, itoa(index, s1, 10)));
