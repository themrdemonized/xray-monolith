...
void CStateGroupDragAbstract::initialize()
{
	...
	object->character_physics_support()->movement()->PHCaptureObject(const_cast<CEntityAlive*>(object->EatedCorpse),
	                                                                 &cb);
	...
	if (capture && !capture->Failed())
	{
		...
		if (m_cover_vertex_id == u32(-1) || object->Position().distance_to(m_cover_position) < 2.f || !object
		                                                                                               ->Home->
		                                                                                               at_min_home(
			                                                                                               m_cover_position)
		)
		{
			const CCoverPoint* point = object->CoverMan->find_cover(object->Home->get_home_point(), 1,
			                                                        object->Home->get_min_radius());
			if (point)
			{
