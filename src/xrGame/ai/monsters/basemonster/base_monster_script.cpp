...
bool CBaseMonster::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
	...
	{
	case eMA_WalkBkwd: anim().m_tAction = ACT_WALK_BKWD;
		break;
	case eMA_Drag: anim().m_tAction = ACT_DRAG;
		break;
	case eMA_Steal: anim().m_tAction = ACT_STEAL;
		break;

	case eMA_WalkWithLeader:
	case eMA_WalkFwd: anim().m_tAction = ACT_WALK_FWD;
		break;

	case eMA_RunWithLeader:
	case eMA_Run: anim().m_tAction = ACT_RUN;
		break;
...
bool CBaseMonster::bfAssignAnimation(CScriptEntityAction* tpEntityAction)
{
	...
	{
	case eAA_StandIdle: anim().m_tAction = ACT_STAND_IDLE;
		break;
	case eAA_CapturePrepare: anim().m_tAction = ACT_CAPTURE_PREPARE;
		break;
	case eAA_SitIdle: anim().m_tAction = ACT_SIT_IDLE;
		break;
	case eAA_LieIdle: anim().m_tAction = ACT_LIE_IDLE;
		break;
	case eAA_Eat: anim().m_tAction = ACT_EAT;
		break;
	case eAA_Sleep: anim().m_tAction = ACT_SLEEP;
		break;
	case eAA_Rest: anim().m_tAction = ACT_REST;
		break;
	case eAA_Attack: anim().m_tAction = ACT_ATTACK;
		break;
	case eAA_LookAround: anim().m_tAction = ACT_LOOK_AROUND;
		break;
	}
...
bool CBaseMonster::bfAssignSound(CScriptEntityAction* tpEntityAction)
{
	...
	{
	case eMonsterSoundIdle: sound().play(eMonsterSoundIdle, 0, 0,
	                                     (l_tAction.m_monster_sound_delay == int(-1))
		                                     ? db().m_dwIdleSndDelay
		                                     : l_tAction.m_monster_sound_delay);
		break;
	case eMonsterSoundEat: sound().play(eMonsterSoundEat, 0, 0,
	                                    (l_tAction.m_monster_sound_delay == int(-1))
		                                    ? db().m_dwEatSndDelay
		                                    : l_tAction.m_monster_sound_delay);
		break;
	case eMonsterSoundAggressive: sound().play(eMonsterSoundAggressive, 0, 0,
	                                           (l_tAction.m_monster_sound_delay == int(-1))
		                                           ? db().m_dwAttackSndDelay
		                                           : l_tAction.m_monster_sound_delay);
		break;
	case eMonsterSoundAttackHit: sound().play(eMonsterSoundAttackHit);
		break;
	case eMonsterSoundTakeDamage: sound().play(eMonsterSoundTakeDamage);
		break;
	case eMonsterSoundDie: sound().play(eMonsterSoundDie);
		break;
	case eMonsterSoundThreaten: sound().play(eMonsterSoundThreaten, 0, 0,
	                                         (l_tAction.m_monster_sound_delay == int(-1))
		                                         ? db().m_dwAttackSndDelay
		                                         : l_tAction.m_monster_sound_delay);
		break;
	case eMonsterSoundSteal: sound().play(eMonsterSoundSteal, 0, 0,
	                                      (l_tAction.m_monster_sound_delay == int(-1))
		                                      ? db().m_dwAttackSndDelay
		                                      : l_tAction.m_monster_sound_delay);
		break;
	case eMonsterSoundPanic: sound().play(eMonsterSoundPanic, 0, 0,
	                                      (l_tAction.m_monster_sound_delay == int(-1))
		                                      ? db().m_dwAttackSndDelay
		                                      : l_tAction.m_monster_sound_delay);
		break;
