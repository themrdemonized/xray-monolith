...
void CTelekineticObject::update_state()
{
	...
	case TS_Raise: raise_update();
		break;
	case TS_Keep: keep_update();
		break;
	case TS_Fire: fire_update();
		break;
	case TS_None: break;
	}
}
...
void CTelekineticObject::switch_state(ETelekineticState new_state)
{
	...
	switch (new_state)
	{
	case TS_Raise: time_raise_started = time;
		break;
	case TS_Keep: time_keep_started = time;
		break;
	case TS_Fire: time_fire_started = time;
		break;
	case TS_None: break;
	}
	state = new_state;
