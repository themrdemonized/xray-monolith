...
void CControlAnimationBase::SetTurnAnimation()
{
	...
	if (m_object->control().path_builder().is_moving_on_path() && m_object->path().enabled() && (delta_yaw >
		MOVE_TURN_ANGLE))
	{
		m_object->SetTurnAnimation(turn_left);
		return;
...
void CControlAnimationBase::SelectVelocities()
{
	...
	if (b_moving)
	{
		u32 cur_point_velocity_index = m_object->movement().detail().path()[m_object
		                                                                    ->movement().detail().
		                                                                    curr_travel_point_index()].velocity;
		...
		if (m_object->movement().detail().path().size() > m_object->movement().detail().curr_travel_point_index() + 1)
			next_point_velocity_index = m_object->movement().detail().path()[m_object
			                                                                 ->movement().detail().
			                                                                 curr_travel_point_index() + 1].velocity;
		...
		if ((cur_point_velocity_index == MonsterMovement::eVelocityParameterStand) && (next_point_velocity_index !=
			u32(-1)))
		{
		...
		const CDetailPathManager::STravelParams& current_velocity = m_object
		                                                            ->movement().detail().velocity(
			                                                            cur_point_velocity_index);
	...
	else
	{
		...
		if (m_tAction == ACT_ATTACK)
		{
			...
			m_object->dir().set_heading_speed(vel * m_object->m_melee_rotation_factor);
			// todo: make as an external factor
		}
		else
