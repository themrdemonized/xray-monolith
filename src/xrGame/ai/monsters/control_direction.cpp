...
void CControlDirection::update_frame()
{
	...
	if (!fis_zero(m_man->movement().velocity_current()) && !fis_zero(m_man->movement().velocity_target()) && m_data.
		linear_dependency)
		m_heading.current_speed = m_data.heading.target_speed * m_man->movement().velocity_current() / (m_man
		                                                                                                ->movement().
		                                                                                                velocity_target()
			+ EPS_L);
	else
	...
	// update pitch
	velocity_lerp(m_pitch.current_speed, m_data.pitch.target_speed, m_pitch.current_acc,
	              m_object->client_update_fdelta());
	...
	if (!m_object->animation_movement_controlled())
		m_object->XFORM().setHPB(-m_man->path_builder().m_body.current.yaw, -m_man->path_builder().m_body.current.pitch,
		                         0);
...
void CControlDirection::pitch_correction()
{
	...
	{
		const DetailPathManager::STravelPathPoint cur_point = m_object->movement().detail().path()[m_object
		                                                                                           ->movement().detail()
		                                                                                           .curr_travel_point_index()
		];
		const DetailPathManager::STravelPathPoint next_point = m_object->movement().detail().path()[m_object
		                                                                                            ->movement().
		                                                                                            detail().
		                                                                                            curr_travel_point_index()
			+ 1];
