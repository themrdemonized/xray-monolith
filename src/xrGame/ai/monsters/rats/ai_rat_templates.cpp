...
IC bool CAI_Rat::bfCheckIfOutsideAIMap(Fvector& tTemp1)
{
	...
	if (!ai().level_graph().valid_vertex_id(dwNewNode) || !ai().level_graph().inside(*ai_location().level_vertex(),
	                                                                                 QueryPos))
	{
...
void CAI_Rat::move(bool bCanAdjustSpeed, bool bStraightForward)
{
	...
	if (m_bNoWay && (!m_turning || (angle_difference(movement().m_body.target.yaw, movement().m_body.current.yaw) <
		EPS_L)))
	{
...
void CAI_Rat::select_next_home_position()
{
	...
	for (; i != e; ++i)
		for (int j = 0; j < iPointCount; ++j)
			if (ai().game_graph().mask(movement().locations().vertex_types()[j].tMask,
			                           ai().game_graph().vertex((*i).vertex_id())->vertex_type()) && ((*i).vertex_id()
				!= m_current_graph_point))
				++iBranches;
	ai().game_graph().begin(tGraphID, i, e);
	if (!iBranches)
	{
	...
	else
	{
		...
		for (; i != e; ++i)
		{
			for (int j = 0; j < iPointCount; ++j)
				if (ai().game_graph().mask(movement().locations().vertex_types()[j].tMask,
				                           ai().game_graph().vertex((*i).vertex_id())->vertex_type()) && ((*i).
					vertex_id() != m_current_graph_point))
				{
