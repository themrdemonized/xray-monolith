	...
	case eRatActionAttackBegin:
		{
			...
			if (memory().enemy().selected() && memory().enemy().selected()->g_Alive() && (dwTime - m_dwStartAttackTime >
				m_dwHitInterval))
			{
...
float CAI_Rat::evaluate(const CItemManager* manager, const CGameObject* object) const
{
	...
	{
		if ((Device.dwTimeGlobal - entity_alive->GetLevelDeathTime() < m_dwEatCorpseInterval) && (entity_alive->m_fFood
			> 0) && (m_bEatMemberCorpses || (entity_alive->g_Team() != g_Team())) && (m_bCannibalism || (entity_alive->
			CLS_ID != CLS_ID)))
