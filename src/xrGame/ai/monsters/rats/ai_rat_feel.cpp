...
void CAI_Rat::feel_sound_new(CObject* who, int eType, CSound_UserDataPtr user_data, const Fvector& Position,
                             float power)
{
	if (!g_Alive())
		return;
	...
	if (power >= m_fSoundThreshold)
	{
		if ((this != who) && ((m_tLastSound.dwTime <= m_dwLastUpdateTime) || (m_tLastSound.fPower <= power)))
		{
			...
			else if (((eType & SOUND_TYPE_WEAPON_SHOOTING) == SOUND_TYPE_WEAPON_SHOOTING) && !memory()
			                                                                                  .enemy().selected())
