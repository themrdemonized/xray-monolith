...
void CPsyDog::Load(LPCSTR section)
{
	...
	if (pSettings->line_exist(section, "Phantoms_Count"))
		...
		m_max_phantoms_count = pSettings->r_u8(section, "Max_Phantoms_Count");
	...
	if (pSettings->line_exist(section, "Time_Phantom_Respawn"))
...
bool CPsyDog::spawn_phantom()
{
	...
	//Alundaio:
	LPCSTR phantomSection = READ_IF_EXISTS(pSettings, r_string, this->get_section(), "phantom_section",
	                                       "psy_dog_phantom");
	CSE_Abstract* phantom = Level().spawn_item(phantomSection, ai().level_graph().vertex_position(node), node, 0xffff,
	                                           true);
	//Alundaio: END
...
void CPsyDogPhantom::Think()
{
	...
	Actor()->Cameras().AddCamEffector(xr_new<CMonsterEffectorHit>(m_appear_effector.ce_time,
	                                                              m_appear_effector.ce_amplitude,
	                                                              m_appear_effector.ce_period_number,
	                                                              m_appear_effector.ce_power));
	Actor()->Cameras().AddPPEffector(xr_new<CMonsterEffector>(m_appear_effector.ppi, m_appear_effector.time,
	                                                          m_appear_effector.time_attack,
	                                                          m_appear_effector.time_release));
}
