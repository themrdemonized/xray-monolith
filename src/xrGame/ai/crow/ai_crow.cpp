...
void CAI_Crow::shedule_Update(u32 DT)
{
	...
	if (st_target != st_current)
	{
		switch (st_target)
		{
		case eFlyUp: switch2_FlyUp();
			break;
		case eFlyIdle: switch2_FlyIdle();
			break;
		case eDeathFall: switch2_DeathFall();
			break;
	...
	switch (st_current)
	{
	case eFlyIdle: if (Position().y > vOldPosition.y) st_target = eFlyUp;
		break;
...
void CAI_Crow::CreateSkeleton()
{
	...
	m_pPhysicsShell->SetMaterial(
		smart_cast<IKinematics*>(Visual())->LL_GetData(smart_cast<IKinematics*>(Visual())->LL_GetBoneRoot()).
		                                    game_mtl_idx);
}
