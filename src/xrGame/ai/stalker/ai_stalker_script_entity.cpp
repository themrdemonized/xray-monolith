...
bool CAI_Stalker::bfAssignWatch(CScriptEntityAction* tpEntityAction)
{
	...
	case CScriptWatchAction::eGoalTypeObject:
		{
			...
			{
				CBoneInstance& l_tBoneInstance = smart_cast<IKinematics*>(l_tWatchAction.m_tpObjectToWatch->Visual())->
					LL_GetBoneInstance(
						smart_cast<IKinematics*>(l_tWatchAction.m_tpObjectToWatch->Visual())->LL_BoneID(
							l_tWatchAction.m_bone_to_watch));
				Fmatrix l_tMatrix;
	...
	if ((CScriptWatchAction::eGoalTypeWatchType != l_tWatchAction.m_tGoalType) && (
		angle_difference(movement().m_head.target.yaw, movement().m_head.current.yaw) < EPS_L) && (angle_difference(
		movement().m_head.target.pitch, movement().m_head.current.pitch) < EPS_L))
		l_tWatchAction.m_bCompleted = true;
	else
