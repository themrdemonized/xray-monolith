...
u32 dwfChooseAction(u32 dwActionRefreshRate, float fMinProbability0, float fMinProbability1, float fMinProbability2,
                    float fMinProbability3, u32 dwTeam, u32 dwSquad, u32 dwGroup, u32 a0, u32 a1, u32 a2, u32 a3,
                    u32 a4, CEntity* tpEntity, float fGroupDistance)
{
	...
	if (!tpEntity)
		...
		{
			if (Group.members()[k]->g_Alive() && ((Group.members()[k]->spatial.type & STYPE_VISIBLEFORAI) ==
				STYPE_VISIBLEFORAI))
	...
	WRITE_QUERY_TO_LOG("\nNew query");
	if (bfGetActionSuccessProbability(Members, VisibleEnemies, fMinProbability0,
	                                  *ai().ef_storage().m_pfVictoryProbability))
	{
	...
	else if (bfGetActionSuccessProbability(Members, VisibleEnemies, fMinProbability1,
	                                       *ai().ef_storage().m_pfVictoryProbability))
	{
		Group.m_dwLastActionTime = Device.dwTimeGlobal;
	...
	else if (bfGetActionSuccessProbability(Members, VisibleEnemies, fMinProbability2,
	                                       *ai().ef_storage().m_pfVictoryProbability))
	{
		Group.m_dwLastActionTime = Device.dwTimeGlobal;
	...
	else if (bfGetActionSuccessProbability(Members, VisibleEnemies, fMinProbability3,
	                                       *ai().ef_storage().m_pfVictoryProbability))
	{
		Group.m_dwLastActionTime = Device.dwTimeGlobal;
