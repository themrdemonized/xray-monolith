...
)
{
	...
	else
	{
		...
		if ((start_circle.radius + dest_circle.radius > distance) && !fsimilar(
			start_circle.radius + dest_circle.radius, distance, EPS_S))
			return (false);
	...
	if (coincide_directions(start_circle.center, tangents[0].point, start_cp, dest_circle.center, tangents[1].point,
	                        dest_cp))
	{
...
)
{
	...
	return (path
		        ...
		        >(vertex_id, start.point, dest.point, *path, t, false, false)
...
{
	if (ai().level_graph().valid_vertex_position(ai().level_graph().v3d(point.position)) && ai()
	                                                                                        .level_graph().inside(
		                                                                                        point.vertex_id,
		                                                                                        point.position))
		return;
...
)
{
	...
	for (int _i = 0, i = 0, n = (int)level_path.size() - 1, j = n, m = j; _i < n;)
	{
		...
		{
			if ((j == n) && !ai().level_graph().valid_vertex_id(
				ai().level_graph().check_position_in_direction(
		...
		if (i >= m - 1)
		{
			...
			if (i == n)
			{
				if (ai().level_graph().valid_vertex_id(
					ai().level_graph().check_position_in_direction(
						start_point.vertex_id, start_point.position, dest.position)))
				{
...
)
{
	...
	do
	{
		...
		if (ai().level_graph().valid_vertex_id(vertex_id))
		{
			...
			if (!reverse_order)
				test_vertex_id = ai()
				                 .level_graph().check_position_in_direction(vertex_id, direction21, point2.position);
			else
				test_vertex_id = ai()
				                 .level_graph().check_position_in_direction(vertex_id, direction21, point0.position);
...
)
{
	...
	if ((B == E) && !compute_path(s, dest, &m_path, m_start_params, finish_params, straight_line_index,
	                              straight_line_index_negative))
	{
		m_path.clear();
		return;
	}
...
)
{
	...
	START_PROFILE("Build Path/Detail Path")
		...
		postprocess_key_points(level_path, intermediate_index, start, dest, finish_params, straight_line_index,
		                       straight_line_index_negative);
