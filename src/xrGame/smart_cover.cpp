...
smart_cover::loophole* cover::best_loophole(Fvector const& position, float& value, bool const& use_default_behaviour,
                              ...
                              float& value, bool const is_smart_cover_entered) const
{
	...
	float const min_enemy_distance = is_smart_cover_entered
		                                 ? object().enter_min_enemy_distance()
		                                 : object().exit_min_enemy_distance();
...
void cover::evaluate_loophole_for_default_usage(Fvector const& position, smart_cover::loophole* & source,
...
static bool in_fov(Fvector const& position, Fvector const& fov_position, Fvector const& fov_direction,
                   float const fov_angle)
{
	Fvector direction = Fvector().sub(position, fov_position);
	if (direction.magnitude() < 1.f)
		return (false);
...
bool cover::in_min_acceptable_range(smart_cover::loophole const& source, Fvector const& position,
