...
u32 CTrade::GetItemPrice(PIItem pItem, bool b_buying, bool b_free)
{
	...
	if (b_buying)
	{
		exponent = pThis.inv_owner->trade_parameters().buy_item_exponent;
	}
	else
	{
	...
	if (buying)
	{
		if (! pThis.inv_owner->trade_parameters().enabled(CTradeParameters::action_buy(0), pItem->object().cNameSect()))
			return 0;
		p_trade_factors = &pThis.inv_owner->trade_parameters().factors(CTradeParameters::action_buy(0),
	...
	else
	{
		...
		p_trade_factors = &pThis.inv_owner->trade_parameters().factors(CTradeParameters::action_sell(0),
		                                                               pItem->object().cNameSect());
	}
