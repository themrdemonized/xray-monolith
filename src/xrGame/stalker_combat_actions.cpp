...
void CStalkerActionGetItemToKill::initialize()
{
	...
	object().sight().setup(
		CSightAction(object().m_best_found_item_to_kill ? &object().m_best_found_item_to_kill->object() : 0,
		             true));
...
void CStalkerActionMakeItemKilling::initialize()
{
	...
	object().sight().add_action(eSightActionTypeWatchItem,
	                            xr_new<CSightControlAction>(
		                            1.f, 3000, CSightAction(SightManager::eSightTypePathDirection)));
	object().sight().add_action(eSightActionTypeWatchEnemy,
	                            xr_new<CSightControlAction>(
		                            1.f, 3000, CSightAction(SightManager::eSightTypePosition,
		                                                    object().memory().enemy().selected()->Position(), false)));
...
void CStalkerActionRetreatFromEnemy::execute()
{
	...
	if (mem_object.m_object)
	{
		...
		point = ai().cover_manager().best_cover(object().Position(), 30.f, *object().m_ce_far,
		                                        CStalkerMovementRestrictor(m_object, true));
		if (!point)
		{
			object().m_ce_far->setup(mem_object.m_object_params.m_position, 0.f, 300.f);
			point = ai().cover_manager().best_cover(object().Position(), 50.f, *object().m_ce_far,
			                                        CStalkerMovementRestrictor(m_object, true));
		}
	}
...
_edge_value_type CStalkerActionRetreatFromEnemy::weight(const CSConditionState& condition0,
...
CStalkerActionGetReadyToKill::CStalkerActionGetReadyToKill(bool affect_properties, CAI_Stalker* object,
                                                           LPCSTR action_name) :
	inherited(object, action_name)
{
	...
	if (object().memory().enemy().selected() && object().memory().enemy().selected()->human_being())
	{
		if (object().agent_manager().member().can_cry_noninfo_phrase())
			if (object().Position().distance_to_sqr(object().memory().enemy().selected()->Position()) < _sqr(10.f))
				if (object().memory().visual().visible_now(object().memory().enemy().selected()) && object()
				                                                                                    .agent_manager().
				                                                                                    member().
				                                                                                    group_behaviour())
...
void CStalkerActionDetourEnemy::initialize()
{
	...
	if (object().memory().enemy().selected() && object().memory().enemy().selected()->human_being() && object()
	                                                                                                   .agent_manager().
	                                                                                                   member().
	                                                                                                   group_behaviour()
	)
...
void CStalkerActionPostCombatWait::initialize()
{
	...
	else
	{
		if (object().inventory().ItemFromSlot(INV_SLOT_2))
		{
			...
			if (object().inventory().ActiveItem() && temp && (object().inventory().ActiveItem()->object().ID() == temp->
				ID()))
	...
	if (object().memory().enemy().last_enemy() && object().memory().visual().visible_now(
		object().memory().enemy().last_enemy()))
...
void CStalkerCombatActionThrowGrenade::execute()
{
	...
	Fvector const head_direction = Fvector().setHP(-object().movement().m_head.current.yaw,
	...
	select_queue_params(distance, min_queue_size, max_queue_size, min_queue_interval, max_queue_interval);
	object().CObjectHandler::set_goal(eObjectActionFire1, &grenade->object(), min_queue_size, max_queue_size,
	                                  min_queue_interval, max_queue_interval);
}
