                                              ...
                                              float time_delta, float object_velocity, float luminocity) const
{
	...
	// SkyKi: Artificially speeds up how quickly AI recognizes enemies that step into their field of view
	const float visibility_boost = g_alife_combat_overhaul ? 2.0f : 1.0f; // SkyKi
	...
	if (ai().script_engine().functor("visual_memory_manager.get_visible_value", funct))
		return (funct(m_object ? m_object->lua_game_object() : 0, game_object ? game_object->lua_game_object() : 0,
		              ...
		              object_velocity, distance, object_distance, always_visible_distance) * visibility_boost); // SkyKi
	//-Alundaio
