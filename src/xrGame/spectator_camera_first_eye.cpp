...
void CSpectrCameraFirstEye::Move(int cmd, float val, float factor)
{
	...
	switch (cmd)
	{
	case kDOWN: pitch -= val ? val : (rot_speed.y * m_fTimeDelta / factor);
		break;
	case kUP: pitch += val ? val : (rot_speed.y * m_fTimeDelta / factor);
		break;
	case kLEFT: yaw -= val ? val : (rot_speed.x * m_fTimeDelta / factor);
		break;
	case kRIGHT: yaw += val ? val : (rot_speed.x * m_fTimeDelta / factor);
		break;
	}
