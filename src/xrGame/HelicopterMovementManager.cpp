...
void SHeliMovementState::UpdatePatrolPath()
{
	if (AlreadyOnPoint())
	{
		...
		parent->callback(GameObject::eHelicopterOnPoint)(dist, currP,
		                                                 currPatrolVertex ? currPatrolVertex->vertex_id() : -1);
		CPatrolPath::const_iterator b, e;
		currPatrolPath->begin(currPatrolVertex, b, e);
		if (b != e)
		{
			if (need_to_del_path && currPatrolVertex->data().flags())
				//fake flags that signals entrypoint for round path
...
void SHeliMovementState::goByRoundPath(Fvector center_, float radius_, bool clockwise_)
{
	...
	for (; it != it_e; ++it, ++pt_idx)
	{
		...
		CPatrolPoint pt = CPatrolPoint((CLevelGraph*)0, (CGameLevelCrossTable*)0, (CGameGraph*)0, pp, (*it).point,
		                               u32(-1), 0, pt_name);
