...
bool CSpaceRestriction::accessible(u32 level_vertex_id, float radius)
{
	...
	return (
		(
			m_out_space_restriction ? m_out_space_restriction->inside(level_vertex_id, false, radius) : true
		)
		&&
		(
...
CSpaceRestriction::CBaseRestrictionPtr CSpaceRestriction::merge(CBaseRestrictionPtr bridge,
                                                                const RESTRICTIONS& temp_restrictions) const
{
...
void CSpaceRestriction::initialize()
{
	...
	if ((m_out_space_restriction && !m_out_space_restriction->initialized()) || (m_in_space_restriction && !
		m_in_space_restriction->initialized()))
		return;
...
bool CSpaceRestriction::affect(SpaceRestrictionHolder::CBaseRestrictionPtr bridge, u32 start_vertex_id,
                               float radius) const
{
	Fsphere sphere;
