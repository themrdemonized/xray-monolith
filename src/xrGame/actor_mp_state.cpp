...
void actor_mp_state_holder::write(NET_Packet& packet)
{
	...
	if (check(physics_linear_velocity_x_flag)) packet.w_float_q8(m_state.physics_linear_velocity.x,
	                                                             min_linear_velocity_component,
	                                                             max_linear_velocity_component);
	if (check(physics_linear_velocity_y_flag)) packet.w_float_q8(m_state.physics_linear_velocity.y,
	                                                             min_linear_velocity_component,
	                                                             max_linear_velocity_component);
	...
	//packet.w_float		(m_state.camera_roll					);
	...
	if (check(inventory_active_slot_flag)) ::write(inventory_active_slot_bits, m_state.inventory_active_slot, current,
	                                               output);
	...
	if (check(physics_state_enabled_flag)) ::write(physics_state_enabled_bits, m_state.physics_state_enabled, current,
	                                               output);
...
void actor_mp_state_holder::read(NET_Packet& packet)
{
	...
	if (check(physics_linear_velocity_x_flag)) packet.r_float_q8(m_state.physics_linear_velocity.x,
	                                                             min_linear_velocity_component,
	                                                             max_linear_velocity_component);
	if (check(physics_linear_velocity_y_flag)) packet.r_float_q8(m_state.physics_linear_velocity.y,
	                                                             min_linear_velocity_component,
	                                                             max_linear_velocity_component);
	if (check(physics_linear_velocity_z_flag)) packet.r_float_q8(m_state.physics_linear_velocity.z,
	                                                             min_linear_velocity_component,
	                                                             max_linear_velocity_component);
	...
	if (check(inventory_active_slot_flag)) m_state.inventory_active_slot = ::read(
		inventory_active_slot_bits, current, output);
	...
	if (check(physics_state_enabled_flag)) m_state.physics_state_enabled = ::read(
		physics_state_enabled_bits, current, output);
}
