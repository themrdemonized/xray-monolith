...
void game_cl_ArtefactHunt::shedule_Update(u32 dt)
{
	...
	if (m_game_ui->m_pBuySpawnMsgBox->IsShown())
	{
		if (m_phase != GAME_PHASE_INPROGRESS || (!local_player || !local_player->testFlag(
...
{
	if (!local_player || local_player->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD) || !local_player->testFlag(
		GAME_PLAYER_FLAG_ONBASE)) return;
