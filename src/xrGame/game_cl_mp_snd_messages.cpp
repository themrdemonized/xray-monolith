...
void game_cl_mp::PlaySndMessage(u32 ID)
{
	...
	for (u32 i = 0; i < m_pSndMessagesInPlay.size(); i++)
	{
		...
		if (pSndMsgIP->priority == SndMsg->priority)
		{
			if (Level().timeServer_Async() > pSndMsgIP->LastStarted + iFloor(
				pSndMsgIP->pSound.get_length_sec() * 1000.0f)) continue;

			u32 Delay = pSndMsgIP->LastStarted + iFloor(pSndMsgIP->pSound.get_length_sec() * 1000.0f) - Level().
				timeServer_Async();
			if (Delay > MaxDelay)
			{
				MaxDelay = Delay;
