...
#pragma todo("insert here loophole selection")
	...
	object().sight().setup(CSightAction(SightManager::eSightTypePosition,
	                                    *object().movement().current_params().cover_fire_position(), true));
	object().sight().update();
...
void loophole_action::initialize()
{
	...
	ActionAnimations const& animations = object().movement().current_params().cover_loophole()->action_animations(
		m_action_id, animation_id);
...
void transition::initialize()
{
	...
	TransitionData const& animations = object().movement().current_params().cover_loophole()->transition_animations(
		m_action_from, m_action_to);
	m_animation = animations[m_random.randI(animations.size())];
}
...
void loophole_fire::execute()
{
	...
	Animations const& animations = object().movement().current_params().cover_loophole()->action_animations(
		m_action_id, animation_id);
	m_animation = animations[m_random.randI(animations.size())];
