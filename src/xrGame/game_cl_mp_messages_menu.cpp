	...
	for (u32 i = 0; i < 10; i++)
	{
		...
		for (u32 s = 1; s <= 16; s++)
		{
			...
			for (int t = 1; t <= GetTeamCount(); t++)
			{
				...
				if (FS.exist(fn, "$game_sounds$", FileName_Voice, ".ogg") && FS.exist(
					fn, "$game_sounds$", FileName_Radio, ".ogg"))
				{
					pNewTeamSound->push_back(cl_Message_Sound());
					cl_Message_Sound* pMsgSound = &(pNewTeamSound->back());
					pMsgSound->mSound_Voice.create(FileName_Voice, st_Effect, sg_SourceType);
					//Msg("-- %s Loaded", FileName_Voice);
					pMsgSound->mSound_Radio.create(FileName_Radio, st_Effect, sg_SourceType);
					//Msg("-- %s Loaded", FileName_Radio);
				}
				else
				{
					R_ASSERT(0);
...
void game_cl_mp::OnSpeechMessage(NET_Packet& P)
{
	...
	if (ps->team == local_player->team)
	{
		CStringTable st;
		if (CurrentGameUI())
			CurrentGameUI()->m_pMessagesWnd->AddChatMessage(st.translate(pMMessage->pMessage.c_str()).c_str(),
			                                                ps->getName());
