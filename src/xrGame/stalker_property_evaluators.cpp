...
CStalkerPropertyEvaluatorEnemyReached::
CStalkerPropertyEvaluatorEnemyReached(CAI_Stalker* object, LPCSTR evaluator_name) :
	inherited(object ? object->lua_game_object() : 0, evaluator_name)
{
}
...
_value_type CStalkerPropertyEvaluatorShouldThrowGrenade::evaluate()
{
	...
	if (!m_storage->property(eWorldPropertyInCover) && !m_storage->property(eWorldPropertyPositionHolded) && !m_storage
		->property(eWorldPropertyEnemyDetoured))
		return (false);
