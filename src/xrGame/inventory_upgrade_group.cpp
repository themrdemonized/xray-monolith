	...
	namespace upgrade
	{
		...
		void Group::construct(const shared_str& group_id, UpgradeBase& parent_upgrade, Manager& manager_r)
		{
			...
			VERIFY2(pSettings->section_exist( m_id ),
			        make_string( "Upgrade <%s> : group section [%s] does not exist!" , parent_upgrade.id_str(), m_id.
				        c_str() ));
			...
			for (int n = _GetItemCount(upgrades_str), i = 0; i < n; ++i)
			{
				UpgradeBase* upgrade_p = (UpgradeBase*)manager_r.add_upgrade(
					_GetItem(upgrades_str, i, temp, buffer_size), *this);
		...
		{
			if (std::find(m_parent_upgrades.begin(), m_parent_upgrades.end(), &parent_upgrade) == m_parent_upgrades.
				end())
			{
