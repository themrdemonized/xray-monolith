...
namespace award_system
{
	...
	void event_conditions_collection::add_event(event_condition_t* root_condition, u32 const max_rise_count,
	                                            u32 const game_id_mask, u32 const delegate_argument)
	{
	...
	bool event_conditions_collection::execute_condition(event_condition_t* cond)
	{
		...
		{
		case eo_logical_and: result = logical_and(cond->m_arguments);
			break;
		case eo_logical_or: result = logical_or(cond->m_arguments);
			break;
		case eo_hit_params: result = hit_params(cond->m_arguments);
			break;
		case eo_kill_params: result = kill_params(cond->m_arguments);
			break;
		case eo_accumul_value_params: result = accumul_params(cond->m_arguments);
			break;
		}; //switch (cond->m_operation)
		return result;
	}
