...
void CWeapon::UpdateZoomParams() {
	...
	} else //Main Sight
	{
		...
		} else if (ALife::eAddonPermanent != m_eScopeStatus && 0 != (m_flagsAddOnState & CSE_ALifeItemWeapon::eWeaponAddonScope) && m_scopes.size())
		{
			...
			if (m_modular_attachments) {
				...
				// pip non-modular hosts read the scope dynamic zoom too, OR so weapon setups still hold
	...
	// pip the base merges a variant scope max zoom to 0, recover it from the _s parent
...
void CWeapon::SetZoomType(u8 new_zoom_type)
{
	...
	Device.m_SecondViewport.dlss_reset_next = true; // pip DLSS history reset on magnification change
	UpdateSecondVP();
}
...
void CWeapon::UpdateCL()
{
	...
	// pip ease the dynamic-scope zoom toward m_fZoomTargetFactor so the view glides between steps, g_zoom_smooth 0 is off
...
{
	// pip integrated scopes report no attached scope, the dynamic-zoom check is the real gate
	if (!m_zoom_params.m_bUseDynamicZoom) return;
	const bool smooth = g_zoom_smooth > 0.f;
	float delta, min_zoom_factor;
	float power = scope_radius > 0.0 ? scope_scrollpower : 1;
	// pip when smoothing advance from the target so rapid scrolls accumulate
	...
	{
		// pip analog zoom, step by a fine fraction of the FOV range so any magnification in the scope range is reachable
...
void CWeapon::UpdateSecondVP()
{
	...
	CActor* pActor = smart_cast<CActor*>(H_Parent());
	// pip the scope_debug force-path also requires IsZoomed so a debug cvar can't drive the SVP off-ADS
	Device.m_SecondViewport.SetSVPActive((scope_debug && scope_svp_enabled && IsSecondVPZoomPresent() && IsZoomed())
...
bool CWeapon::GetSVPCameraMatrix(Fmatrix& camera)
{
	if (Device.m_SecondViewport.eyepiece.radius > EPS) {
		// many guns have their mesh directly scaled so the lens is the only reliable unit of measurement
