					...
					while (!e_what->children.empty())
					{
						...
						if (e_child_item)
						{
							if (!OnTouch(eid_who, e_child_item->ID, FALSE))
							{
								...
								m_server->Process_event_reject(P, m_server->GetServerClient()->ID, 0, e_what->ID,
								                               e_child_item->ID);
								continue;
							}
						}
...
void game_sv_Deathmatch::Send_Anomaly_States(ClientID id_who)
{
	...
	{
		u8 AnomalyState = u8(
			(m_dwLastAnomalySetID == j) ? CCustomZone::eZoneStateIdle : CCustomZone::eZoneStateDisabled);
