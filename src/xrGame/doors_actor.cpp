...
)
{
	...
	float const min_distance = std::min(distance_to_diagonal,
	                                    std::min(distance_to_open_state, distance_to_closed_state));
...
bool actor::update_doors(doors_type const& detected_doors, float const average_speed)
{
	...
	temp_doors_type new_doors_to_open(_alloca(detected_doors_count * sizeof(doors_type::value_type)),
	                                  detected_doors_count);
	temp_doors_type new_doors_to_close(_alloca(detected_doors_count * sizeof(doors_type::value_type)),
	                                   detected_doors_count);
	...
	for (; i != e; ++i)
	{
		...
		float const distance_to_open = movement.is_going_through(matrix, (*i)->get_vector(door_state_open),
		                                                         check_distance);
		float const distance_to_closed = movement.is_going_through(matrix, (*i)->get_vector(door_state_closed),
		                                                           check_distance);
		if (distance_to_open >= 0.f)
		{
			if (distance_to_closed >= 0.f)
				...
				{
					if (!add_new_door(average_speed, *i, m_open_doors, m_closed_doors, new_doors_to_open,
					                  door_state_open))
						return false;
				}
				else
				{
					if (!add_new_door(average_speed, *i, m_closed_doors, m_open_doors, new_doors_to_close,
					                  door_state_closed))
						return false;
				}
			else
