	...
	virtual void Execute(LPCSTR args)
	{
		...
		else
		{
			...
			if (sscanf_s(args, "%u", &tmp_client_id) != 1)
			{
				...
				Msg(
					"Kick player. Format: \"sv_kick_id <player session id | \'%s\'>\". To receive list of players ids see sv_listplayers",
	...
	virtual void Execute(LPCSTR args_)
	{
		...
		else
		{
			...
			if (sscanf_s(args_, "%u", &tmp_client_id) != 1)
			{
				Msg("! ERROR: bad command parameters.");
				Msg(
					"Make screenshot. Format: \"make_config_dump <player session id | \'%s\'> <ban_time_in_sec>\". To receive list of players ids see sv_listplayers",
					LAST_PRINTED_PLAYER_STR
				);
				return;
			}
...
public:
	...
	virtual void Execute(LPCSTR args_)
	{
		...
		if (sscanf_s(args_, "%s %i", &hex_digest, sizeof(hex_digest), &ban_time) != 2)
		{
			Msg("! ERROR: bad command parameters.");
			Msg(
				"Ban player. Format: \"sv_banplayer_by_digest <hex digest> <ban_time_in_sec>\". To get player hex digest you can enter: sv_listplayers_banned");
			return;
		}
	...
	virtual void Execute(LPCSTR args_)
	{
		...
		else
		{
			...
			if (sscanf_s(args_, "%u", &player_index) != 1)
			{
				Msg("! ERROR: bad command parameters.");
				Msg(
					" Unban player. Format: \"sv_unbanplayer <banned player index | \'%s\'>. To receive list of banned players se sv_listplayers_banned",
