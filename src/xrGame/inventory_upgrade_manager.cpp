...
namespace inventory
{
	namespace upgrade
	{
		...
		{
			VERIFY2(pSettings->section_exist( item_id ),
			        make_string( "Inventory item [%s] does not exist!", item_id.c_str() ));
		...
		void Manager::load_all_properties()
		{
			...
			VERIFY2(pSettings->section_exist( properties_section ),
			        make_string( "Section [%s] does not exist !", properties_section ));
			VERIFY2(pSettings->line_count( properties_section ),
	...
	if ( g_upgrades_log == 1 )
	{
		...
		Upgrade* Manager::upgrade_verify(shared_str const& item_section, shared_str const& upgrade_id)
		{
			...
			VERIFY2(root_p->contain_upgrade( upgrade_id ),
			        make_string( "Inventory item <%s> not contain upgrade <%s> !", item_section.c_str(), upgrade_id.
				        c_str() ));
