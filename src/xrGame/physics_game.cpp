...
private:
	...
	virtual bool compare(const CPHLiquidParticlesPlayCall* v) const
	{
		...
		return disp.x * disp.x + disp.z * disp.z < (minimal_plane_distance_between_liquid_particles *
			minimal_plane_distance_between_liquid_particles);
	}
};
                    ...
                    const SGameMtl* static_mtl, LPCSTR ps_name)
{
	...
	if (play_not_liquid)
		Level().ph_commander().add_call(xr_new<CPHOnesCondition>(),
		                                xr_new<CPHParticlesPlayCall>(*c, b_invert_normal, ps_name));
	else if (play_liquid)
	{
	...
	if (data)
	{
		...
		if (mtl_pair)
		{
			...
			if (vel_cret > Pars::vel_cret_wallmark && !mtl_pair->m_pCollideMarks->empty())
			{
				...
				//ref_shader pWallmarkShader = mtl_pair->CollideMarks[::Random.randI(0,mtl_pair->CollideMarks.size())];
				Level().ph_commander().add_call(xr_new<CPHOnesCondition>(),
				                                xr_new<CPHWallMarksCall>(*((Fvector*)c->pos), T, WallmarkShader));
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if (square_cam_dist < SQUARE_SOUND_EFFECT_DIST)
			{
				...
				if (!static_mtl->Flags.test(SGameMtl::flPassable))
				{
					if (vel_cret > Pars::vel_cret_sound)
					{
						...
						{
							float volume = collide_volume_min + vel_cret * (collide_volume_max - collide_volume_min) / (
