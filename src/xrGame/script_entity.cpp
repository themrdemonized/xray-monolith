...
void CScriptEntity::SetScriptControl(const bool bScriptControl, shared_str caSciptName)
{
	...
	))
	{
		ai().script_engine().script_log(eLuaMessageTypeError,
		                                "Invalid sequence of taking an entity under script control");
		return;
	}
                                              ...
                                              const Fvector& tAngleOffset)
{
	...
	{
		CBoneInstance& l_tBoneInstance = smart_cast<IKinematics*>(object().Visual())->LL_GetBoneInstance(
...
#endif
				const Fmatrix& l_tMatrix = GetUpdatedMatrix(l_tSoundAction.m_caBoneName,
				                                            l_tSoundAction.m_tSoundPosition,
				                                            l_tSoundAction.m_tSoundAngles);
		...
		if (true/** !l_tParticleAction.m_tpParticleSystem/**/)
			if (!l_tParticleAction.m_bStartedToPlay)
			{
				const Fmatrix& l_tMatrix = GetUpdatedMatrix(*l_tParticleAction.m_caBoneName,
				                                            l_tParticleAction.m_tParticlePosition,
...
bool CScriptEntity::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
	...
	case CScriptMovementAction::eGoalTypePathPosition:
		{
			...
			u32 vertex_id;
			vertex_id = ai().level_graph().vertex(object().ai_location().level_vertex_id(),
	...
	case CScriptMovementAction::eGoalTypeNoPathPosition:
		{
			m_monster->movement().set_path_type(MovementManager::ePathTypeLevelPath);
			if (m_monster->movement().detail().path().empty() || (m_monster->movement().detail().path()[m_monster
			                                                                                            ->movement().
			                                                                                            detail().path().
			                                                                                            size() - 1]
