...
Fmatrix& CIKFoot::foot_to_ref_bone_transform(Fmatrix& m) const
{
	...
	m.mul_43(Fmatrix().invert(Kinematics()->LL_GetTransform(m_foot_bone_id)),
...
ik_goal_matrix::e_collide_state CIKFoot::CollideFoot(float angle, float& out_angle, const Fvector& global_toe,
                                                     const Fvector& foot_normal, const Fvector& global_bone_pos,
...
bool CIKFoot::make_shift(Fmatrix& xm, const Fvector& cl_point, bool collide, const Fplane& p,
                         const Fvector& pick_dir) const
{
...
bool CIKFoot::GetFootStepMatrix(ik_goal_matrix& m, const SCalculateData& cd, const SIKCollideData& cld, bool collide,
                                bool rotate) const
{
	return GetFootStepMatrix(m, cd.state.anim_pos, cld, collide, rotate);
}

ik_goal_matrix::e_collide_state CIKFoot::rotate(Fmatrix& xm, const Fplane& p, const Fvector& foot_normal,
                                                const Fvector& global_point, bool collide) const
{
	Fvector ax;
                                ...
                                bool rotation, bool b_make_shift/*=true*/) const
{
	...
	if (b_make_shift && make_shift(xm, local_point, collide, p, cld.m_pick_dir))
		...
		case ik_goal_matrix::cl_free:
			cl_state = ik_goal_matrix::cl_translational;
			break;
		case ik_goal_matrix::cl_rotational:
			cl_state = ik_goal_matrix::cl_mixed;
			break;
		default: NODEFAULT;
		}
