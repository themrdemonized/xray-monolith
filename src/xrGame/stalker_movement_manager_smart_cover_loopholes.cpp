...
) const
{
	...
	for (; i != e; ++i)
	{
		...
		for (; I != E; ++I)
		{
			...
			if (target_body_state && ((*I)->body_state() != *target_body_state) && (result_body_state == *
				target_body_state))
				continue;
	...
	VERIFY2(
		...
		make_string(
			...
			loophole_id0.c_str(),
			loophole_id1.c_str(),
			!target_body_state ? "" : (*target_body_state == eBodyStateStand ? "stand" : (*target_body_state ==
				eBodyStateCrouch ? "crouch" : "invalid!")),
			VPUSH(position)
		)
	);
	return (*result);
...
void stalker_movement_manager_smart_cover::build_exit_path()
{
	...
	for (; I != E; ++I)
	{
		...
		float exit_edge = cur_cover.description()->transitions().edge(exitable_loophole_id,
		                                                              smart_cover::transform_vertex("", false))->
		                            weight();
		new_value += exit_edge;

		u32 targe_vertex_id = level_dest_vertex_id();
		Fvector target_position = m_target.desired_position()
			                          ? *m_target.desired_position()
...
void stalker_movement_manager_smart_cover::build_exit_path_to_cover()
{
	...
	for (; I != E; ++I)
	{
		...
		float new_value = ai().graph_engine().m_string_algorithm->data_storage().get_best().g();
		float exit_edge = current_cover.description()->transitions().edge(
			exitable_loophole_id, smart_cover::transform_vertex("", false))->weight();
		new_value += exit_edge;
...
void stalker_movement_manager_smart_cover::actualize_path()
{
	...
	VERIFY(m_current.cover());
	shared_str current_loophole_id = smart_cover::transform_vertex(
		m_current.cover_loophole() ? m_current.cover_loophole()->id() : "", true);
...
void stalker_movement_manager_smart_cover::try_actualize_path()
{
	...
	shared_str current_loophole_id = smart_cover::transform_vertex(
		m_current.cover() ? m_current.cover_loophole()->id() : "", true);
