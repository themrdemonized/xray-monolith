...
#define EPSILON 0.000001f
	...
	else if (e < -1.0f + EPSILON)
	{
		...
		float fxx = -from.x * from.x;
		float fyy = -from.y * from.y;
		float fzz = -from.z * from.z;
		float fxy = -from.x * from.y;
		float fxz = -from.x * from.z;
		float fyz = -from.y * from.z;
		...
		float lxx = -Left.x * Left.x;
		float lyy = -Left.y * Left.y;
		float lzz = -Left.z * Left.z;
		float lxy = -Left.x * Left.y;
