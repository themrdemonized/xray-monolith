...
static xr_set<xr_string> verified_paths;
...
void VerifyPath(LPCSTR path)
{
    ...
    for (int i = 0; path[i]; i++)
    {
        ...
        if (verified_paths.find(tmp) != verified_paths.end())
            continue;

        _mkdir(tmp);
        verified_paths.insert(tmp);
    }
}
...
{
    HANDLE hFile = CreateFile(file_name, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, 0);
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        if (size > 0)
        {
            ...
            if (hMap)
            {
                ...
                if (mapped)
                {
                    void* buffer = Memory.mem_alloc(size
#ifdef DEBUG_MEMORY_NAME
                        , "FILE in memory"
#endif
                    );
                    CopyMemory(buffer, mapped, size);
...
void IReader::r_stringZ(char* dest, u32 tgt_sz)
{
    char* src = (char*)data;
    char* start = src + Pos;
    char* end = (char*)memchr(start, 0, Size - Pos);
    if (end)
    {
        u32 sz = (u32)(end - start);
        R_ASSERT2(sz < tgt_sz, "Dest string less than needed.");
        CopyMemory(dest, start, sz);
        dest[sz] = 0;
        Pos += sz + 1;
    }
    else
    {
        u32 sz = 0;
        while ((src[Pos] != 0) && (!eof())) { *dest++ = src[Pos++]; sz++; }
        *dest = 0;
        Pos++;
    }
}
