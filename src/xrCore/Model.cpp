...
inline void PPM_CONTEXT::encodeBinSymbol(int symbol)
{
	...
	{
		rcBinCorrect1(tmp, BIN_SCALE - bs);
...
inline void PPM_CONTEXT::encodeSymbol1(int symbol)
{
	...
	while ((++p)->Symbol != symbol)
	{
		...
		if (--i == 0)
		{
			...
			FoundState = NULL;
			do { CharMask[(--p)->Symbol] = EscCount; }
			while (--i);
			SubRange.high = SubRange.scale;
			return;
		}
	}
