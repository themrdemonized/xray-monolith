...
void CEnvDescriptor::load(CEnvironment& environment, CInifile& config)
{
	...
	if (config.line_exist(m_identifier.c_str(), "sky_rotation")) sky_rotation = deg2rad(
		config.r_float(m_identifier.c_str(), "sky_rotation"));
	else sky_rotation = 0;
...
void CEnvironment::load_weathers()
{
	...
	for (; i != e; ++i)
	{
		...
		if (!((length >= 4) && ((*i)[length - 4] == '.') && ((*i)[length - 3] == 'l') && ((*i)[length - 2] == 't') && ((*i)[length - 1] == 'x')))
			continue;
