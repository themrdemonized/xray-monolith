...
public:
	...
	Flags32 m_Flags;
public:
	PropItem(EPropType _type): type(_type), prop_color(0), val_color(0), item(0), key(0), OnClickEvent(0),
	                           OnDrawTextEvent(0), OnItemFocused(0) { m_Flags.zero(); }
...
public:
	...
	NumericValue(T* val, T mn, T mx, T increm, int decim): CustomValue<T>(val), lim_mn(mn), lim_mx(mx), inc(increm),
	                                                       dec(decim)
	{
...
public:
	FLAG_TYPE mask;
public:
	FlagValue(T* val, FLAG_TYPE _mask, LPCSTR c0, LPCSTR c1, u32 flags): CustomValue<T>(val),
	                                                                     FlagValueCustom(flags, c0, c1), mask(_mask)
	{
	}
