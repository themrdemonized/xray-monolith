	...
	struct CMatcher
	{
		...
		struct CMatchHelper2
		{
			...
			struct CMatchHelper3
			{
				...
				struct selector
				{
					...
					{
						typedef Loki::Typelist<typename PrevHead::Head, Loki::Typelist<
							                       Head, Loki::Typelist<Target, Loki::NullType>>> result;
					};
		...
		struct CMatchHelper
		{
			...
			typedef typename selector<
				object_type_traits::is_base_and_derived<typename Head::Head, Source>::value || object_type_traits::
				is_same<typename Head::Head, Source>::value
			>::result result;
		};
...
IC T1 smart_cast(T2* p)
{
	...
	STATIC_CHECK(
		object_type_traits::is_void<object_type_traits::remove_pointer<T1>::type>::value || is_polymorphic<
		object_type_traits::remove_pointer<T1>::type>::result, Invalid_target_type_for_Dynamic_Cast);
...
IC T1 smart_cast(T2& p)
{
	...
	STATIC_CHECK(is_polymorphic<object_type_traits::remove_reference<T1>::type>::result,
