...
BOOL IPureClient::Connect(LPCSTR options)
{
	...
	if (!psNET_direct_connect)
	{
		...
		HRESULT CoCreateInstanceRes = CoCreateInstance(CLSID_DirectPlay8Client, NULL, CLSCTX_INPROC_SERVER,
		                                               IID_IDirectPlay8Client, (LPVOID*)&NET);
		//---------------------------	
    ...
    R_CHK(NET->Initialize	(this, Handler, 0));
		...
		net_Address_device = NULL;
		R_CHK(CoCreateInstance (CLSID_DirectPlay8Address,NULL, CLSCTX_INPROC_SERVER, IID_IDirectPlay8Address,(LPVOID*) &
			net_Address_device ));
		...
		if (stricmp(server_name, "localhost") == 0)
		{
			...
			if (dpServerDesc->pwszSessionName)
			{
				...
				R_CHK(WideCharToMultiByte(CP_ACP,0,dpServerDesc->pwszSessionName,-1,dpSessionName,sizeof(dpSessionName),
					0,0));
