...
int SetBackTrajectoryCnt(const dReal* p, const dReal* last_pos, Triangle& neg_tri, dxGeom* o1, dxGeom* o2,
...
)
{
	...
	if (*pushing_neg)
	{
		...
		neg_tri_contains_point = TriContainPoint(neg_vertices[0], neg_vertices[1], neg_vertices[2], neg_tri.norm,
		                                         neg_tri.side0,
		                                         neg_tri.side1, p);
			...
			if ((!(last_pos_dist < 0.f)) || b_pushing)
				if (__aabb_tri(Point(p), Point((float*)&AABB), vertices))
				{
					...
					)
					{
						...
						if (neg_depth > tri.depth && (!(*pushing_neg || spushing_neg) || dDOT(neg_tri.norm, tri.norm) >
							-M_SQRT1_2) && (!(*pushing_b_neg || spushing_b_neg) || dDOT(b_neg_tri.norm, tri.norm) > -
							M_SQRT1_2))
							//exclude switching on opposite side &&(!*pushing_b_neg||dDOT(b_neg_tri->norm,tri.norm)>-M_SQRT1_2)
						{
							neg_depth = tri.depth;
							neg_tri = tri;
							data->neg_tri = tri.T;
					...
					if (!b_pased && b_passable)
					{
						...
						dReal sidePr = primitive.Proj(o1, tri.norm);
						tri.depth = sidePr - tri.dist;
						if (b_neg_depth > tri.depth && (!(*pushing_b_neg || spushing_b_neg) ||
							dDOT(b_neg_tri.norm, tri.norm) > -M_SQRT1_2) && ((!*pushing_neg || !spushing_neg) ||
							dDOT(neg_tri.norm, tri.norm) > -M_SQRT1_2))
						{
							//exclude switching on opposite side &&(!*pushing_neg||dDOT(neg_tri->norm,tri.norm)>-M_SQRT1_2)
