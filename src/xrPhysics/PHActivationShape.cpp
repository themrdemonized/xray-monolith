...
static void ActivateTestDepthCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                                      SGameMtl* material_2)
{
	if (!do_colide ||
...
void StaticEnvironment(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1, SGameMtl* material_2)
{
	...
	if (bo1)
	{
		((CPHActivationShape*)(retrieveGeomUserData(c.geom.g1)->callback_data))
			->DActiveIsland()->ConnectJoint(contact_joint);
...
void CPHActivationShape::Create(const Fvector start_pos, const Fvector start_size, IPhysicsShellHolder* ref_obj,
