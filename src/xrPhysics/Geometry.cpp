...
void TransformedGeometryExtensionLocalParams(dGeomID geom_transform, const dReal* axis, float center_prg,
                                             dReal* local_axis, dReal& local_center_prg)
{
	R_ASSERT2(dGeomGetClass(geom_transform)==dGeomTransformClass, "is not a geom transform");
...
void CBoxGeom::set_build_position(const Fvector& ref_point)
{
	...
	dVector3 local_position = {
		m_box.m_translate.x - ref_point.x,
