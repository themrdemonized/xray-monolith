...
#pragma warning(disable:4245)
			if (material_1->Flags.test(SGameMtl::flSlowDown) && !(!usr_data_2 || usr_data_2->pushing_neg || usr_data_2->
				pushing_b_neg))
#pragma warning(pop)
			{
...
#pragma warning(push)
#pragma warning(disable:4245)
			if (material_2->Flags.test(SGameMtl::flSlowDown) && !(!usr_data_1 || usr_data_1->pushing_neg || usr_data_1->
				pushing_b_neg))
#pragma warning(pop)
			{
		...
		if (usr_data_2)
		{
			usr_data_2->pushing_b_neg = usr_data_2->pushing_b_neg && !GMLibrary()
			                                                          .GetMaterialByIdx(usr_data_2->b_neg_tri->material)
			                                                          ->Flags.test(SGameMtl::flPassable);
			usr_data_2->pushing_neg = usr_data_2->pushing_neg && !GMLibrary()
			                                                      .GetMaterialByIdx(usr_data_2->neg_tri->material)->
			                                                      Flags.test(SGameMtl::flPassable);
		...
		if (usr_data_1)
		{
			usr_data_1->pushing_b_neg = usr_data_1->pushing_b_neg && !GMLibrary()
			                                                          .GetMaterialByIdx(usr_data_1->b_neg_tri->material)
			                                                          ->Flags.test(SGameMtl::flPassable);
			usr_data_1->pushing_neg = usr_data_1->pushing_neg && !GMLibrary()
			                                                      .GetMaterialByIdx(usr_data_1->neg_tri->material)->
			                                                      Flags.test(SGameMtl::flPassable);
...
float E_NL(dBodyID b1, dBodyID b2, const dReal* norm)
{
	...
	if (b1)
	{
		if (b2)return E_NLD(b1, b2, norm);
		else return E_NlS(b1, norm, 1);
	}
