...
IC u8 TransferenceAndThrowVelToThrowDir(const Fvector& transference, float throw_vel, float gravity_accel,
                                        Fvector throw_dir[2])
{
	...
	case 0: return 0;
		break;
	case 2: throw_dir[1].y = tgA.y * s;
		throw_dir[1].normalize();
	case 1: throw_dir[0].y = tgA.x * s;
