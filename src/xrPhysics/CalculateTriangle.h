...
ICF float DistToTri(Triangle* T, const float* pos, float* dir, float* p, ETriDist& c, const Fvector* V_array)
{
	...
	switch (code)
	{
	case 1: tdist = DistToFragmenton(pos, VRT[0], VRT[1], p, dir, cd);
		break;
	case 2: tdist = DistToFragmenton(pos, VRT[1], VRT[2], p, dir, cd);
		break;
	case 3: tdist = DistToFragmenton(pos, VRT[2], VRT[0], p, dir, cd);
		break;
	default: NODEFAULT;
	}
	switch (cd)
