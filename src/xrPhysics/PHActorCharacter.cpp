...
void CPHActorCharacter::InitContact(dContact* c, bool& do_collide, u16 material_idx_1, u16 material_idx_2)
{
	...
	if (b_single_game)
	{
		...
		if (b_restrictor &&
			do_collide &&
			!(b1
				  ? static_cast<CPHCharacter*>(retrieveGeomUserData(c->geom.g2)->ph_object)->ActorMovable()
	...
	else
	{
		...
		if (D1 && D2)
		{
			...
			{
				do_collide = do_collide && !b_restrictor && (A1->ObjectPPhysicsShell() == 0) == (A2->
					ObjectPPhysicsShell() == 0);
				c->surface.mu = 1.f;
			}
		}
