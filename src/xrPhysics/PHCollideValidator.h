...
public:
	enum ETypeFlags
	{
		...
		cbClassAnimated = 1 << 10,
		//класс анимированного физического объекта
		cbNCClassAnimated = 1 << 11,
		//свойство игнорирования класса анимированного физического объекта
		cbNone = 1 << 12
	};
...
private:
	...
	static IC bool DoCollideNonMatched(const CPHObject& obj1, const CPHObject& obj2)
	{
		return
			!(
				((obj1.collide_class_bits().flags & ClassNCFlags.flags) & ((obj2.collide_class_bits().flags & ClassFlags
					.flags) << 1))
				||
				((obj2.collide_class_bits().flags & ClassNCFlags.flags) & ((obj1.collide_class_bits().flags & ClassFlags
					.flags) << 1))
			);
	}
