...
CPhysicsShellAnimator(CPhysicsShell* _pPhysicsShell, CInifile const* ini, LPCSTR section) : m_pPhysicsShell(
	_pPhysicsShell)
{
	...
	if (all_bones)
		for (xr_vector<CPHElement*>::iterator i = m_pPhysicsShell->Elements().begin(); i != m_pPhysicsShell
		                                                                                    ->Elements().end(); i++)
			CreateJoint(*i);
...
void CPhysicsShellAnimator::CreateJoints(LPCSTR controled)
{
	...
	for (u16 i = 0; nb > i; ++i)
	{
		...
		VERIFY2(bid != BI_NONE,
		        make_string( "shell_animation - controled bone %s not found! object: %s, model: %s", n, obj->ObjectName(
		        ), obj->ObjectNameVisual()));
		CPHElement* e = smart_cast<CPHElement*>(m_pPhysicsShell->get_Element(bid));
		VERIFY2(
			e, make_string( "shell_animation - controled bone %s has no physics collision! object: %s, model: %s", n,
...
void CPhysicsShellAnimator::CreateJoint(CPHElement* e)
{
	...
	((CPHShell*)(m_pPhysicsShell))->Island().DActiveIsland()->AddJoint(
		PhysicsShellAnimatorBoneDataC.m_anim_fixed_dJointID);
	dJointAttach(PhysicsShellAnimatorBoneDataC.m_anim_fixed_dJointID,
	             PhysicsShellAnimatorBoneDataC.m_element->get_body(), 0);
