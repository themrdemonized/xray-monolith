...
slaxml.parser = function(self, callbacks)
    return {_call = callbacks or _defcall, parse = self.parse}
end
...
local function parse_include_line(line)
...
local function parse_includes(s, rootFile)
    local res = ""
    if rootFile then
...
function slaxml.parse(self, xml, options)
    if not options then
...
function slaxml.dom(self, xml, opts)
    if not opts then
        opts = {}
    end
...
function slaxml.xml(self, n, opts)
    opts = opts or {}
    local out = {}
        ...
        if n.attr and n.attr[1] then
            ...
            for _, a in ipairs(sorted) do
                if (not a.nsURI or not omit[a.nsURI]) and not (omit[a.value] and a.name:find("^xmlns:")) then
                    attrs[#attrs + 1] =
                        " " .. (a.nsPrefix and (a.nsPrefix .. ":") or "") .. a.name .. '="' .. esc(a.value) .. '"'
                end
            end
...
function slaxml.simple_dom(self, xml, opts)
    if not opts then
        opts = {}
    end
    local push, pop = table.insert, table.remove
...
function slaxml.sanitize_attr_list(self, s)
    local res
...
-- opts.indent: number of spaces, or string
function slaxml.simple_xml(self, n, opts)
    opts = opts or {}
    local out = {}
    local tab = opts.indent and (type(opts.indent) == "number" and string.rep(" ", opts.indent) or opts.indent) or ""
    local ser = {}
