...
local options_builder = require("options/builder")
local page = options_builder.page
local line = options_builder.line
local button = options_builder.button
local track = options_builder.track
local list_bool = options_builder["list-bool"]
local list_enum = options_builder["list-enum"]

 -- instance, don't touch
local HDRLiveEditingGUI = nil
...
local function start_hdr_live_editing_ui(self)
    self:On_Cancel()
    start()
end
...
local PAGE = page(
   ...
   button(
      ...
      [[functor_ui]], function(self)
         return start_hdr_live_editing_ui(self)
      end,
      [[precondition]], function()
         return level_present() and get_console_cmd(1, "r4_hdr10_on")
      end
   ),
   track(
      [[id]], "whitepoint_nits",
...
function HDRLiveEditingControl:__init()
   super()
   self:InitControls()
   ...
   for i=1, #hdr_live_editing_commands do
     ...
     -- self.Num[i]:SetWndPos(vector2():set(num_pos.x + 250, num_pos.y))
      ...
      if tbl.type == "list" then
         for _, v in pairs(tbl.content[1]()) do
            if v[1] == string_value then
               display_text = game.translate_string(
                  ("ui_mm_lst_%s"):format(v[2])
               )
               break
            end
         end
