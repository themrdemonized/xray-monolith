...
# Only Vulkan renderer will be used
...
Write-Host "  (Excluding R1/R2/R3/R4 renderers)" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Green
Write-Host ""

# Build projects in order, EXCLUDING R1/R2/R3/R4
$projects = @(
    "xrCore",
    "xrCDB", 
    "xrSound",
    "xrScriptEngine",
    "xrNetServer",
    "xrRender_Vulkan",  # ONLY Vulkan renderer
    "xrEngine"
)

foreach ($proj in $projects) {
    Write-Host ">>> Building $proj..." -ForegroundColor Yellow
    & $MSBuild $Solution /t:$proj /p:Configuration=$Config /p:Platform=$Platform /v:minimal /nologo /m:4
